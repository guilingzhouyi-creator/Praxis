...
Boundary note (constants governance):
  Prompt template strings are DATA managed by this registry — they live in
  ``_DEFAULTS`` and ``config/praxis.yaml`` prompts:, NOT in params/*.py.
  This is intentional: templates are user-overridable runtime content
  (registry + config overrides), whereas params/*.py holds compile-time
  magic values (timeouts, limits, truncation lengths, path strings).
  Do not copy prompt bodies into params; add new templates here or via
  the prompts: config section instead.
"""
...
_DEFAULTS: dict[str, str] = {
    ...
    # Identity binding (Phase 1) — built-in generalized minimal role fragment.
    # Custom strict-role fragments are config/API-provided, never hardcoded here.
    "identity_binding.default_fragment": (
        "You are bound to role '{role}' in cell '{cell_id}'. Follow the role's "
        "restrictions and stay within the assigned domain."
    ),
    # Generic three-identity fields (A2): intent keywords per identity,
    # used by htn_planner.match_identity for HTN-C task dispatch. Values are
    # '|'-separated keyword patterns (lowercase substring match); overridable
    # via config/discovery/identity_roles.yaml → prompts: section.
    ...
    # ── R4 agent failure reflection / distillation ──
    "r4_agent.failure_analyst": ("You are a failure analyst for an agent operating system."),
    "r4_agent.skill_architect_system": ("You are a skill architect."),
    "r4_agent.reflect_patterns": (
        "These are repeated failures for one tool. Reflect on the root "
        "cause and the fix, naming a canonical pattern.\n{digest}\n"
        'Reply with JSON only: {{"why": "<root cause>", '
        '"fix": "<how to fix>", "pattern": "<pattern-name>"}}'
    ),
    "r4_agent.summarize_lesson": (
        "Distill these failure patterns for the tool into ONE concise "
        "reusable lesson.\n{digest}\n"
        'Reply with JSON only: {{"lesson": "<one paragraph>"}}'
    ),
    "r4_agent.distill_patterns": (
        "You are a skill architect. Distill these failure patterns for "
        "the tool '{tool}' into a structured skill definition:\n{digest}\n"
        "{verified_context}\n"
        'Reply with JSON only: {{"name": "<tool>_lessons", '
        '"description": "<one line>", "prompt": "<procedural guidance>", '
        '"rules": ["DO: ...", "DONT: ..."], "procedures": [{{"step": "..."}}]}}'
    ),
    # ── Discussion / subagent / memory-graph prompts ──
    "discussion.answer_review": (
        "You are {examiner}. Review the answers from {target}.\n\n"
        "{target}'s answers:\n{target_content}\n\n"
        "Cross-examine: identify gaps, inconsistencies, or areas "
        "that need clarification."
    ),
    "l3a.subagent.system": (
        "You are a {spec_name} subagent for L3A. "
        "Use allowed tools: {allowed_tools}. "
    ...
    "agent_loop.run_code_usage": (
        ...
        "program against the generated tool SDK below and submit it with "
        "run_code. The program may loop, branch, and fan out across tool "
        "calls in a single execution; only its printed output and return "
        "value are returned to you. Every tool call the program makes is "
