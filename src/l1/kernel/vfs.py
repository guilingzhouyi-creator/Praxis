...
from .params import system as _sys_params
...
class VFS:
    ...
    def __init__(self):
        ...
        # Split locks: mounts vs cache vs virtual files so the hot read path
        # (cache lookup) does not serialize against mount/unmount operations.
    ...
    def _prune_cache_locked(self, now: float) -> None:
        """Evict expired cache entries; if still over FILE_CACHE_MAX_ENTRIES, drop oldest.

        Caller must hold ``self._cache_lock``. Without pruning, the cache dict
        grows without bound as distinct paths are read — expired entries
        were previously only skipped on lookup, never removed.
        """
        expired = [p for p, (_, exp) in self._cache.items() if now >= exp]
        for p in expired:
            self._cache.pop(p, None)
        # dict preserves insertion order — the first key is the oldest entry
        while len(self._cache) > _sys_params.FILE_CACHE_MAX_ENTRIES:
            self._cache.pop(next(iter(self._cache)))

    def _prune_virtual_locked(self) -> None:
        """Cap the virtual-file dict at FILE_CACHE_MAX_ENTRIES (oldest evicted first)."""
        while len(self._virtual_files) > _sys_params.FILE_CACHE_MAX_ENTRIES:
            self._virtual_files.pop(next(iter(self._virtual_files)))

    def unlink(self, path: str, agent_ring: int = VFS_DEFAULT_MIN_RING) -> dict:
        """Delete a file through VFS — removes a virtual file or the real file.

        Returns a success dict with the removed path, or ENOENT / EACCES /
        EROFS errors matching the read/write error vocabulary.
        """
        mp, rel, real = self._resolve(path)
        if not mp:
            return {"success": False, "error": f"ENOENT: no mount for '{path}'", "error_code": "ENOENT"}
        ...
        if mp.mount_type == MountType.VIRTUAL:
            with self._virtual_lock:
                if path not in self._virtual_files:
                    return {"success": False, "error": f"ENOENT: {path}", "error_code": "ENOENT"}
                del self._virtual_files[path]
            return {"success": True, "mount": mp.name, "path": path}
        try:
            os.remove(real)
            return {"success": True, "mount": mp.name, "path": real}
    ...
    def proc_path(self) -> str:
        """Path of the mounted proc filesystem."""
    ...
    def unmount(self, name: str) -> dict:
        ...
        with self._lock:
            ...
            self._sorted_prefixes = sorted(self._mounts, key=lambda p: -len(p))
        # Drop cached entries under this mount (separate lock, not nested)
        with self._cache_lock:
            for k in list(self._cache):
