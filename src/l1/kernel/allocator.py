...
class Allocator:
    ...
    def __init__(self):
        ...
        # Per-agent lock shards: single-agent operations lock only their
        # shard (hash(agent_id) % N), so parallel agents do not serialize.
        # Structural changes to the shared dicts (new-agent inserts,
        # cleanup pops) are guarded by the global lock (_ensure_agent /
        # cleanup_agent) so cross-agent iterators never race them.
        # Cross-agent scans (pressure/_oom_kill/summary) iterate under the
        # global lock, then release it before touching any per-agent shard —
        # global→shard ordering is forbidden (would invert the shard→global
        # path used by _ensure_agent and the OOM reclaim flow).
        self._shards: list[threading.RLock] = [threading.RLock() for _ in range(ALLOCATOR_SHARD_COUNT)]
    ...
    def _lock_for(self, agent_id: str) -> threading.RLock:
        """Return the shard lock for an agent (stable hash → shard)."""
        return self._shards[hash(agent_id) % ALLOCATOR_SHARD_COUNT]

    def _ensure_agent(self, agent_id: str) -> None:
        """Register an agent's accounting entries (limits, allocations, counter).

        Structural inserts into the three shared dicts happen under the global
        lock so cross-agent iterators (pressure/_oom_kill/summary) never see
        the outer dicts mutate mid-iteration; the lock-free fast path (agent
        already tracked) keeps the common case shard-only. Callers may hold a
        shard lock — shard→global is the documented allowed ordering.
        """
        if agent_id in self._allocations:
            return
        with self._lock:
            self._limits.setdefault(agent_id, dict(self.DEFAULTS))
            self._allocations.setdefault(agent_id, [])
            self._usage_counter.setdefault(agent_id, {})
    ...
    def alloc(
        ...
        with self._lock_for(agent_id):
            ...
            counter = self._usage_counter.get(agent_id, {})
            used = counter.get(resource, 0)
            limit = self._limits.get(agent_id, self.DEFAULTS).get(resource, ALLOCATOR_FALLBACK_LIMIT)
        ...
        # ── Cross-agent OOM path (no shard lock held) ──
        # _oom_kill selects the victim under the global lock and reclaims
        # under the *victim's* shard; holding the requester's shard here too
        # would let two OOM-ing agents deadlock on each other's shards.
        ...
        with self._lock_for(agent_id):
            ...
            used = counter.get(resource, 0)
            limit = self._limits.get(agent_id, self.DEFAULTS).get(resource, ALLOCATOR_FALLBACK_LIMIT)
            available = limit - used + victim_freed
            if available < amount:
                return {
                    "success": False,
    ...
    def usage(self, agent_id: str) -> dict:
        ...
        Read-only: an unknown agent yields default-profile stats without
        inserting registry entries (queries must not grow state).
        """
        with self._lock_for(agent_id):
            limits = self._limits.get(agent_id, dict(self.DEFAULTS))
            counter = self._usage_counter.get(agent_id, {})
            result = {}
    ...
    def cleanup_agent(self, agent_id: str) -> dict:
        """Drop all accounting state for *agent_id* — limits, allocations, usage counters.

        Called from process reaping so exited agents do not linger in the
        allocator (memory-leak hygiene).
        """
        # Structural pops are global-guarded so cross-agent iterators
        # (pressure/_oom_kill/summary) never race a dict resize.
        with self._lock:
            self._limits.pop(agent_id, None)
            self._allocations.pop(agent_id, None)
            self._usage_counter.pop(agent_id, None)
        return {"success": True}
    ...
    def pressure(self, threshold: float = ALLOCATOR_PRESSURE_THRESHOLD) -> dict:
        ...
        # Cross-agent scan: snapshot agent ids under the global lock, then
        # read per-agent usage via its shard lock — never acquire a shard lock
        # while holding the global lock (lock-order inversion with alloc→OOM).
        with self._lock:
            agent_ids = list(self._limits.keys())
    ...
    def _oom_kill(self, requesting_agent: str, resource: str, needed: int) -> int:
        ...
        with self._lock:
            ...
            # Ensure the victim's entries exist before releasing the global
            # lock — global→shard ordering is forbidden.
            self._ensure_agent(victim)
        reclaim = min(total, needed)

        # Reclaim under the victim's shard so it serializes with the victim's
        # own alloc/free/swap_out paths. The caller (alloc) holds no shard
        # here, so two OOM-ing agents cannot deadlock on each other's shards.
        freed = 0
        with self._lock_for(victim):
            allocs = self._allocations.get(victim, [])
            victim_counter = self._usage_counter.get(victim, {})
            kept: list[Allocation] = []
            for a in allocs:
