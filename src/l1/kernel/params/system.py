...
# ── Tiered cross-cell cache (2.1-D2) ───────────────────────────────────────
# Three-layer cache architecture:
#   L1 — this Cell's hot zone (per-cell, same as IsolatedCache)
#   L2 — Cell-to-Cell shared summaries (HTN-B reads only this tier)
#   L3 — cross-Cell archive index (heavy, oldest evicted first)
TIERED_CACHE_L1_MAX_ENTRIES: Final[int] = 500
TIERED_CACHE_L1_TTL: Final[float] = 60.0
TIERED_CACHE_L2_MAX_ENTRIES: Final[int] = 2000
TIERED_CACHE_L2_TTL: Final[float] = 300.0
TIERED_CACHE_L3_MAX_ENTRIES: Final[int] = 10000
TIERED_CACHE_L3_TTL: Final[float] = 3600.0


# ── Review pipeline (2.1-D3) ───────────────────────────────────────────────
...
# Frontend-heavy mode only: diffs are stitched into a ring buffer written
...
# ── Memory domain filter (Phase 3, M1) ─────────────────────────────────────
# Gate memory retrieval by Agent identity + Cell domain. Both switches are
# operator-controlled (API + L2 Shell), never code-embedded:
#   enabled      — master switch (off = no filtering, backward compatible)
...
# accumulated evidence may lift them into the kept set. Never exceeds the
# high-importance threshold used for promotion.
MEMORY_REFINE_REBURN_BOOST: Final[float] = 3.0
MEMORY_REFINE_REBURN_MAX_SCORE: Final[float] = 40.0
# ── Memory refinery (Phase 3, M2) ─────────────────────────────────────
# Write-path purification pipeline (classify/dedup/clean/score/refine/
# transform). Operator-controlled master switch; off by default so the
# write path stays backward compatible. When enabled, refined records
# are persisted to the TieredCache archive index for the RC memory source.
MEMORY_REFINERY_ENABLED_DEFAULT: Final[bool] = False
# Supply refined records to the generalized skill system (R4Agent input).
# Kept separate from the refinery switch: LLM skill evolution is heavy.
MEMORY_REFINERY_SUPPLY_SKILLS_DEFAULT: Final[bool] = False
...
# Dead-peer grace period before eviction from the peer table (5x timeout)
NET_PEER_EVICT_AFTER: Final[float] = 300.0
...
# Stage-state entries (skill_guidance _stage_state) untouched this long are pruned
SKILL_STAGE_STATE_TTL: Final[float] = 86400.0
# Debounce window for the stage-state prune scan (avoids O(N) on every call)
SKILL_STAGE_PRUNE_INTERVAL: Final[float] = 300.0
...
# Per-domain posture knobs + the master switch. Attack posture is
# default-off; only explicit configuration/API turns it on. The matrix is
# data (config/discovery/posture.yaml overrides), never hardcoded in tools.
POSTURE_MATRIX_DEFAULT: Final[dict[str, dict]] = {
    # domain → {offensive: bool, target_whitelist: [glob...]}
    "build": {"offensive": False, "target_whitelist": []},
    "attack": {"offensive": False, "target_whitelist": []},
}
# Master switch for the posture matrix API surface (config/API enabled)
POSTURE_API_ENABLED: Final[bool] = True
# Harness modes forbidden while any domain is in offensive posture —
# minimal drops approval + rate limiting, which must never combine with
# attack posture (approval is the non-negotiable safety bottom line).
POSTURE_FORBIDDEN_HARNESS: Final[tuple[str, ...]] = ("minimal",)
...
# ── Security evidence chain (posture/harness/skill-gate bypass audit) ────
# chain_id-threaded evidence of attack-posture switching and gate decisions
# (security_mode / harness / offensive-policy / skill injection / use_skill /
# gatechain G4 / constitution §9.2), persisted as append-only JSONL under
# data_dir. Side-channel semantics: recording never breaks the protected path.
SECURITY_EVIDENCE_FILE: Final[str] = "security_evidence.jsonl"  # relative to data_dir
EVIDENCE_CHAIN_MAX_CHAINS: Final[int] = 64  # in-memory chain index cap (oldest evicted)
EVIDENCE_CHAIN_MAX_EVIDENCE: Final[int] = 512  # in-memory evidence window cap (oldest evicted)
EVIDENCE_CHAIN_RAW_MAX: Final[int] = 2048  # raw JSON snapshot chars stored per evidence point
EVIDENCE_CHAIN_HASH_TRUNC: Final[int] = 16  # sha256 prefix length for evidence fixity ids
EVIDENCE_CHAIN_REPORT_FINDINGS_MAX: Final[int] = 32  # max findings in a chain report
...
# Max diagnostics injected into agent context per failed tool call
LSP_DIAG_INJECT_LIMIT: Final[int] = 5
...
RESOURCE_BUFFER_ROOT_DIR: Final[str] = "resource-buffer"
...
L3B_MESSAGE_DIR: Final[str] = "l3b-messages"
...
CI_REVIEW_POLL_INTERVAL: Final[float] = 0.5  # Pipeline status poll interval (s)
