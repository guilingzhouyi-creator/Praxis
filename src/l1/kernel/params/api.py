...
# ── Load-adaptive thread pool controller (load_adaptive.py) ──
# Master switch; false = static pool at MIN (zero behavior change)
LOAD_ADAPTIVE_ENABLED: Final[bool] = True
# Sampling period for load signals (s)
LOAD_ADAPTIVE_SAMPLE_INTERVAL: Final[float] = 1.0
# EWMA smoothing factor (0.0-1.0); higher = more responsive but noisier
LOAD_ADAPTIVE_EWMA_ALPHA: Final[float] = 0.3
# Queue-ratio below which the pool shrinks (multiplicative)
LOAD_ADAPTIVE_LOW_RATIO: Final[float] = 0.2
# Queue-ratio above which the pool grows (additive)
LOAD_ADAPTIVE_HIGH_RATIO: Final[float] = 0.6
# Consecutive out-of-bound samples required before a decision
LOAD_ADAPTIVE_HYSTERESIS_SAMPLES: Final[int] = 3
# Cooldown period after each decision to prevent oscillation (s)
LOAD_ADAPTIVE_COOLDOWN_S: Final[float] = 5.0
# Additive grow step (workers added per growth decision)
LOAD_ADAPTIVE_GROW_STEP: Final[int] = 2
# Multiplicative shrink denominator (workers //= factor, at least MIN)
LOAD_ADAPTIVE_SHRINK_FACTOR: Final[int] = 2
# Slow-task threshold: fraction of TASK_TIMEOUT above which a task is slow
LOAD_ADAPTIVE_SLOW_TASK_RATIO: Final[float] = 0.5
