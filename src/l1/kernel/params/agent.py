...
DEFAULT_AGENT_CONFIGS: Final[dict[str, AgentDefaults]] = {
    "default": AgentDefaults(
        max_scouts=3, max_tokens=4096, max_workers=4, priority=5, ring=1, system_prompt_key="agent_loop.system.default"
    ),
...
CENTRAL_ROLES: list[str] = ["reader", "writer", "reviewer", "scout", "l3", "default", "deployer", "tester"]
CENTRAL_DEFAULT_ROLES: list[str] = ["reader", "writer", "reviewer"]

# ── Generic three-identity fields (build / test / review) ─────────────────
# Every Agent entity is a composite of these three standard identities —
# a single field set, not a hardcoded role. HTN-C task dispatch matches a
# task to one (or more) identities and the matched fragment is injected
# into the session context. Definitions are data: config/discovery/
# identity_roles.yaml can override the built-ins without code changes.
IDENTITY_BUILD: Final[str] = "build"  # construct / modify / implement
IDENTITY_TEST: Final[str] = "test"  # verify / regress / validate
IDENTITY_REVIEW: Final[str] = "review"  # cross-check / audit / approve
# Ordered tuple of the three generic identity fields (canonical order)
IDENTITY_FIELDS: Final[tuple[str, ...]] = (IDENTITY_BUILD, IDENTITY_TEST, IDENTITY_REVIEW)
# Allowed identity values (validated on bind / inject)
IDENTITY_VALID: Final[tuple[str, ...]] = IDENTITY_FIELDS
# Default identity set applied when no config matches an agent/cell
IDENTITY_DEFAULT_SET: Final[tuple[str, ...]] = (IDENTITY_BUILD, IDENTITY_TEST, IDENTITY_REVIEW)

# ── Cell-level identity (B1) — distinct from Agent identities ────────────
# A Cell domain has its own identity surface — NOT the Agent build/test/
# review fields. Departments inherit the default Cell identity and a
# deployment may specialize it (e.g. a testing department sets
# CELL_IDENTITY_TEST). Config-driven via config/discovery/departments.yaml.
CELL_IDENTITY_DEFAULT: Final[str] = "general"  # default Cell domain identity
CELL_IDENTITY_TEST: Final[str] = "test"  # testing-department Cell identity
CELL_IDENTITY_REVIEW: Final[str] = "review"  # review-department Cell identity
CELL_IDENTITY_VALID: Final[tuple[str, ...]] = (
    CELL_IDENTITY_DEFAULT,
    CELL_IDENTITY_TEST,
    CELL_IDENTITY_REVIEW,
)
# Default department type applied when none is configured (inherit default
# Cell settings; users may override per department).
DEPARTMENT_TYPE_DEFAULT: Final[str] = "general"
# Allowed department types (a department is NOT hardcoded to testing —
# testing is one configurable specialization among these).
DEPARTMENT_TYPES: Final[tuple[str, ...]] = ("general", "build", "test", "review", "custom")
...
# ── Identity binding (Phase 1: per-Cell role → prompt-fragment binding) ──
# Structural limits only — prompt content lives in the prompt registry /
# praxis.yaml identity_binding:, never in params (see prompts.py note).
IDENTITY_BINDING_MAX_CHARS: Final[int] = 1200
...
# When enabled AND the Cell count reaches CELL_DEPARTMENT_MIN, the
# organization splits into egalitarian departments (e.g. a professional
# testing department bound to the tester role). Runtime switch:
# settings "departments.enabled".
CELL_DEPARTMENT_MIN: Final[int] = 2
DEPARTMENT_ENABLED_DEFAULT: Final[bool] = False
...
# Stable lifecycle vocabulary shared by Python3 and language-neutral adapters.
