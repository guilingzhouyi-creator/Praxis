...
class ToolHistoryLedger:
    ...
    def _prune_index_keys_locked(self) -> None:
        """Bound the number of distinct index keys (memory hygiene).

        Deques inside each bucket are already maxlen-capped, but the
        dict KEYS (per agent/tool/combination) were never pruned — a
        long-running system with many distinct agents/tools would grow
        the index dicts without bound. Caller must hold ``self._lock``.
        """
        for index in (self._by_agent_tool, self._by_agent, self._by_tool):
            while len(index) > LEDGER_MAX_INDEX_KEYS:
                # Evict the bucket whose most recent entry is oldest.
                oldest_key = min(index, key=lambda k: index[k][-1].timestamp if index[k] else 0.0)
...
class GateChain:
    ...
    def __init__(self):
        ...
        self._gates: tuple[tuple[str, Callable], ...] = tuple(_BUILTIN_GATES)
    ...
    def register_tools(self, tool_names: list[str]) -> None:
        """Add *tool_names* to the set of known tools (copy-on-write)."""
        with self._lock:
    ...
    def register_gate(self, name: str, fn: Callable, index: int | None = None) -> None:
        """Register a gate check.  Insert at *index* (default: append) — copy-on-write."""
        entry = (name, fn)
        with self._lock:
            current = self._gates
            if index is not None:
                self._gates = current[:index] + (entry,) + current[index:]
            else:
                self._gates = current + (entry,)
