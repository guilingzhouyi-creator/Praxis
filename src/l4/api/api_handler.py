...
def _auth_ok(headers, auth_token: str) -> bool:
    ...
    if header.startswith("Bearer "):
        ...
        try:
            ...
            # Auth port unavailable/broken: degrade to the static token
            # instead of silently rejecting — but surface it for ops.
            logger.warning("api_gateway: auth port verify_token failed, falling back to static token")
