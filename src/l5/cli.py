...
def cmd_boot(args) -> dict:
    ...
    if r.get("success"):
        ...
        print(f"  Cell: {r.get('cell_id', DEFAULT_CELL_ID)}")
...
def cmd_backup(args) -> dict:
    """Create a backup snapshot of the runtime data_dir (tag optional)."""
    from l3.services.backup import create_backup

    tag = args[0] if args else ""
    r = create_backup(tag=tag)
    if r.get("success"):
        print(f"✓ Backup created: {r['backup']} ({r['copied_files']} files, {r['bytes']} bytes)")
    else:
        print(f"✗ Backup FAILED: {r.get('error')}")
    return r


def cmd_backups(args) -> dict:
    """List available backup snapshots."""
    from l3.services.backup import list_backups
    ...
    for e in entries:
        print(f"  {e['backup']}: {e['files']} files, {e['bytes']} bytes")
    return {"success": True, "backups": entries}


def cmd_restore(args) -> dict:
    """Restore data_dir from a named backup (destructive). Usage: restore <backup-name>"""
    from l3.services.backup import restore_backup

    if not args:
        print("Usage: restore <backup-name>")
        return {"success": False}
    r = restore_backup(args[0])
    if r.get("success"):
        print(f"✓ Restored {r['restored_files']} files from {r['backup']}")
    else:
        print(f"✗ Restore FAILED: {r.get('error')}")
    return r
