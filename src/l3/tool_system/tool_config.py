...
class ToolConfig:
    ...
    def register_from_dict(cls, name: str, defn: dict, ring: str = "ring_1") -> dict:  # noqa: PLR0911
        ...
        dynamic count cap, then builds and registers the spec. Returns a
        structured result dict — callers (boot YAML scan, L4 API handler)
        surface the error field to the operator.

        Args:
            name: Tool name (must not already be registered).
            defn: Tool definition dict (description/handler/params/danger/...).
            ring: ToolRing value; defaults to ring_1.

        Returns:
            ``{"success": True, "name": ..., "ring": ...}`` or a failure dict.
        """
        from l1.kernel.params.tool import (
            TOOL_REGISTRY_ALLOWED_RINGS,
            TOOL_REGISTRY_DEFAULT_CATEGORY,
            TOOL_REGISTRY_MAX_DYNAMIC,
        )

        from .tool_registry import get_registry

        if ring not in TOOL_REGISTRY_ALLOWED_RINGS:
            return {"success": False, "error": f"ring '{ring}' not allowed for dynamic registration"}
        registry = get_registry()
        if name in registry.all_names():
            return {"success": False, "error": f"tool '{name}' already registered"}
        dynamic = [
            t
            for t in registry.all_names()
            if getattr(registry.get(t), "category", "") == TOOL_REGISTRY_DEFAULT_CATEGORY
        ]
