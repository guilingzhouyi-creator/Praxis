...
def get_mode() -> str:
    """Return the current tool mode (read|write)."""
    return _MODE
...
def set_mode(mode: str) -> dict:
    ...
    with _MODE_LOCK:
        ...
        _MODE = mode
    # Apply and persist OUTSIDE the lock: mode switches are low-frequency,
    # but _save_mode does synchronous file I/O — holding _MODE_LOCK across
    # disk writes would serialize unrelated get_mode() readers on I/O latency.
    _apply_mode()
    _save_mode()
