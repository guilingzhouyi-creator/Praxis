...
class ToolRegistry:
    ...
    def clear(self) -> None:
        """Reset all registry state in place (identity-stable singleton reset).

        Unlike ``reset_registry`` swapping the instance, clearing in place keeps
        ...
        self._middleware = []
        self._muted = set()
        self._muted_categories = set()
        self._muted_plugins = set()
...
def reset_registry() -> None:
    """Reset the module-level ToolRegistry singleton in place.

    Clears the existing instance instead of swapping it, so module-level
    ``TOOL_REGISTRY`` stays identical to ``get_registry()`` — a swap would
    leave captured references pointing at a stale (still-populated) registry,
    which breaks register/unregister symmetry under parallel tests.
    """
    global _registry
    if _registry is None:
        _registry = ToolRegistry()
