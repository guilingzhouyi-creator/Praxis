...
class CentralMemory:
    ...
    def get_memory(self, scope_id: str) -> Any:
        """Get (or lazily create) the isolated memory instance for a scope.

        Generic scope accessor: any scope (``l3a``, ``l3a-c-<n>``, a Cell
        id, ...) gets its own isolated MemoryManager persisted under
        ``memories/<scope_id>/`` — extension-first, no hardcoded scopes.

        Args:
            scope_id: Memory scope name (e.g. "l3a", "l3a-c-1", "cell-1").

        Returns:
            The MemoryManager instance for that scope.
        """
        return self.get_or_create(scope_id)

    def get_l3a_memory(self) -> Any:
        """L3A's own independent R1-R3 memory instance (scope "l3a")."""
...
def get_memory(scope_id: str) -> Any:
    """Convenience: get (or create) the isolated memory for any scope.

    Generic scope accessor mirroring CentralMemory.get_memory — extension-
    first: ``get_l3a_memory()`` is just ``get_memory("l3a")``.
    """
    return get_center().get_or_create(scope_id)
