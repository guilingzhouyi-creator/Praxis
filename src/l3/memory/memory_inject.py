...
class MemoryInjector:
    ...
    def build_context(
        ...
        *,
        ...
        ``intent``/``domain`` carry the driving card's identity-hit so the
        fine-grained memory gate follows task dispatch (identity is not a
        static role); both default to "" (gate falls back to static set).
        """
        ...
        try:
            if task == TASK_DECIDE:
                mer = self._mer_block(agent_id, max_tokens, intent=intent, domain=domain)
                if mer:
                    return mer
                return self._summary_block(agent_id, max_tokens, memory, intent=intent, domain=domain)
            if task == TASK_RESUME:
                return self._layered_block(agent_id, max_tokens, memory, intent=intent, domain=domain)
            return self._summary_block(agent_id, max_tokens, memory, intent=intent, domain=domain)
        except Exception as e:
            logger.debug("memory_inject: build_context failed: %s", e)
            return self._summary_block(agent_id, max_tokens, memory, intent=intent, domain=domain)

    def _summary_block(self, agent_id: str, max_tokens: int, memory=None, intent: str = "", domain: str = "") -> str:
        ...
        try:
            ...
            return m.build_context(agent_id, max_tokens=budget, intent=intent, domain=domain)
        except Exception:
            return ""

    def _mer_block(self, agent_id: str, max_tokens: int, intent: str = "", domain: str = "") -> str:
        ...
        try:
            ...
            seeds = []
            if mem is not None:
                recent = mem.recall(agent_id=agent_id or None, rings=[1, 2, 3], limit=5, intent=intent, domain=domain)
    ...
    def _layered_block(self, agent_id: str, max_tokens: int, memory=None, intent: str = "", domain: str = "") -> str:
        """Skeleton + narrative (layered injection, budget-aware)."""
        mer = self._mer_block(agent_id, max_tokens // 2, intent=intent, domain=domain)
        summary = self._summary_block(agent_id, max_tokens // 2, memory, intent=intent, domain=domain)
...
def build_context(
    ...
    *,
    ...
    return MemoryInjector().build_context(
        agent_id, card=card, prompt=prompt, max_tokens=max_tokens, memory=memory, intent=intent, domain=domain
    )
