...
class MemoryQueryMixin:
    ...
    # Host-provided attributes (declared by MemoryManager)
    _ring: Any
    short: Any
    stats: Any
    working: Any
    ...
    def recall(
        ...
        Args:
            ...
            intent: Driving task intent (HTN-C identity-hit source for the
                    fine-grained memory gate, optional).
            domain: Optional card domain hint for the identity match.
        """
        ...
        # Phase 3 M1: identity/Cell-domain gate on retrieval (operator
        # switches; disabled → unchanged, backward compatible). Routed
        # through the supply-chain re-injection wrapper so the M3 path is
        # exercised in production.
        try:
            from .memory_domain_filter import get_memory_filter
            from .memory_supply_chain import re_inject_filtered

            _f = get_memory_filter()
            if _f.status().get("enabled"):
                kept = {
                    d["id"]
                    for d in re_inject_filtered(
                        [e.to_dict() for e in results],
                        cell_id=cell_id or "",
                        role="",
                        scope="",
                        intent=intent,
                ...
                results = [e for e in results if e.id in kept]
        except Exception:
            logger.debug("memory: domain filter skipped")
        return results
    ...
    def build_context(
        ...
        ``intent``/``domain`` scope the fine-grained identity gate over the
        knowledge block (identity follows card dispatch, not static role).
        """
        return _build_context(self, agent_id, max_tokens=max_tokens, intent=intent, domain=domain)
