...
class SkillLifecycleMixin:
    ...
    def _curate_skills(self) -> int:
        ...
        for ev in evolved:
            if ev["injected"] >= min_trials and ev["contrib"] < min_ratio:
                try:
                    from l3.tools._archive import _cmd_archive_store
                    ...
                    _cmd_archive_store(
                        fonds="skills",
                        series="retired",
                        content=ev["record"].get("prompt", "") or "",
                        tags=f"{ev['name']},evolved,retired,contrib={ev['contrib']:.2f}",
                    )
                except Exception as ex:
                    logger.debug("R4Agent: archive retired skill failed: %s", ex)
                sm.delete(ev["name"], internal=True)
                self._remove_skill_dir(ev["name"])
                logger.info(
