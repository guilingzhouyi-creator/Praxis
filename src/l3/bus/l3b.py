...
class L3BComposite:
    ...
    def route_subtask(self, subtask: Any, prev_summary: str) -> list[Any]:
        """Use HTN-B to decompose an HTN-A subtask into a row plan.

        The subtask is normalized into a compound root task and decomposed
        by the composite's HTN-B planner (route_forward / merge_back
        methods). The flattened primitive plan is serialized into row dicts;
        the preceding Cell's L2 cache summary is attached to check-prev
        steps so the executor can verify prerequisite work. When no routing
        method matches, a single direct-dispatch row is returned instead of
        an empty plan — routing never silently drops a subtask.
        """
        name, domain, tid = self._subtask_fields(subtask)
        try:
            root = self.htn_b.decompose(name, domain=domain, agent_id=self.next_cell)
            plan = self.htn_b.flatten(root)
        except Exception as e:
            logger.warning("%s route_subtask decompose failed: %s", self.composite_id, e)
            plan = []
        # HTNPlanner.decompose never returns an empty plan: an unmatched
        # intent yields a synthesized tool-less "{id}-step-0" primitive.
        # Treat that default as "no routing method matched" and fall through
        # to the direct-dispatch row — otherwise an executor would receive a
        # row it cannot dispatch (no tool, no target_cell).
        if plan and len(plan) == 1 and not plan[0].tool and "step-0" in plan[0].id:
            plan = []
        if not plan:
            # No routing method matched — fall back to direct dispatch so the
            # subtask is handed to the succeeding Cell unchanged.
            return [
                {
                    "id": f"{tid}-route",
                    "name": f"Route {name} to {self.next_cell}",
                    "task_type": "PRIMITIVE",
                    "tool": "dispatch_to_next",
                    "domain": domain,
                    "agent_id": self.next_cell,
                    "params": {"action": "dispatch_card", "target_cell": self.next_cell, "task_name": name},
                    "depends_on": [],
                }
            ]
        rows: list[dict] = []
        for t in plan:
            d: dict[str, Any] = {
                "id": t.id,
                "name": t.name,
                "task_type": t.task_type.name if hasattr(t.task_type, "name") else str(t.task_type),
                "tool": t.tool,
                "domain": t.domain,
                "agent_id": t.agent_id,
            ...
            if "check-prev" in t.id and prev_summary:
                from l1.kernel.params.system import LOG_TRUNC_500

                d["params"]["prev_summary"] = prev_summary[:LOG_TRUNC_500]
            rows.append(d)
        return rows

    def _subtask_fields(self, subtask: Any) -> tuple[str, str, str]:
        """Extract (name, domain, id) from a subtask — Task, dict, or string."""
        if hasattr(subtask, "name"):
            return subtask.name, getattr(subtask, "domain", ""), subtask.id
        if isinstance(subtask, dict):
            return (
                str(subtask.get("name", subtask.get("intent", ""))),
                str(subtask.get("domain", "")),
                str(subtask.get("id", "sub")),
            )
        return str(subtask), "", "sub"
