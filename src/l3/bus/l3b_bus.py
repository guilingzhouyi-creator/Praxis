...
class L3BMessageType(Enum):
    ...
    REVIEW_REWORK = auto()  # 2.1: directed rework request from the review department
...
class L3BBus:
    ...
    def route_to_cell(
        ...
        sender: str,
        target_cell: str,
        msg_type: L3BMessageType,
        payload: Any = None,
    ) -> dict:
        """Auto-route a message to a target Cell across the composite topology (2.1).

        The Cell-to-Cell transport is a topology, not a direct pair: with
        more than two Cells the shortest channel between the sender and the
        target Cell is computed from the registered composites (``l3b-{a}-{b}``,
        adjacent when their Cell sets intersect) via BFS, then the message
        is delivered hop-by-hop into each intermediate composite's mailbox.

        Args:
            sender: Sender composite id (or "" to start from the first
                registered composite — L3A orchestration view).
            target_cell: The Cell the message must reach.
            msg_type: Message type (e.g. REVIEW_REWORK for directed rework).
            payload: Message payload (routing path is appended).

        Returns:
            ``{"success": bool, "path": [composite...], "hops": N}`` or an
            error dict when no composite reaches the target Cell.
        """
        with self._lock:
            composites = list(self._mailboxes.keys())
        if not composites:
            return {"success": False, "error": "no composites registered"}

        def _cells(cid: str) -> set:
            # composite_id format: "l3b-{cell_a}-{cell_b}" — but Cell ids
            # themselves contain "-" (e.g. "cell-1"), so split("-") cannot
            # recover them; match the "cell-<n>" tokens instead.
            import re

            return set(re.findall(r"cell[-_]?\d+", cid))

        # Sender composite: exact match, else composite containing a sender
        # cell, else first registered (L3A orchestration view).
        sender_comp = sender if sender in composites else None
        sender_cells = _cells(sender)
        if sender_comp is None:
            for cid in composites:
                if sender_cells and _cells(cid) & sender_cells:
                    sender_comp = cid
                    break
        if sender_comp is None:
            sender_comp = composites[0]

        targets = [cid for cid in composites if target_cell in _cells(cid)]
        if not targets:
            return {"success": False, "error": f"no composite reaches cell {target_cell}"}

        if sender_comp in targets:
            path = [sender_comp]
        else:
            # Adjacency graph: composites share an edge when they share a Cell.
            adj: dict[str, list[str]] = {cid: [] for cid in composites}
            for a in composites:
                a_cells = _cells(a)
                for b in composites:
                    if a != b and a_cells & _cells(b):
                        adj[a].append(b)
            prev: dict[str, str | None] = {sender_comp: None}
            q = deque([sender_comp])
            found: str | None = None
            while q:
                cur = q.popleft()
                ...
                for nxt in adj.get(cur, []):
                    if nxt not in prev:
                        ...
                        q.append(nxt)
            if found is None:
                return {"success": False, "error": f"no route from {sender_comp} to cell {target_cell}"}
            path = []
            node: str | None = found
        ...
        # Deliver hop-by-hop along the computed channel.
        for i, hop in enumerate(path):
            msg = L3BMessage(
                msg_id=f"route-{time.time_ns()}-{i}",
                msg_type=msg_type,
                sender=sender_comp,
                target=hop,
                payload={**(payload or {}), "route_path": path, "hop": i, "target_cell": target_cell},
            )
            with self._lock:
                mbox = self._mailboxes.get(hop)
                if mbox is not None:
                    mbox.append(msg)
            self._stats["sent"] += 1
        return {"success": True, "path": path, "hops": max(0, len(path) - 1)}
