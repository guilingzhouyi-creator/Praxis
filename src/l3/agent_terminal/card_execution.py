    ...
    def _execute_with_existing_loop(self, card) -> dict:
        ...
        try:
            ...
            if _ptags and self._active_loop is not None:
                ...
                # Card→identity linkage: re-scope the persistent loop's
                # identity channels to the current card (task-intent match +
                # domain-expert fragment read these during prompt assembly).
                self._active_loop._task_intent = task or ""
                self._active_loop._task_domain = _domain
                self._active_loop._card_domain = _domain
        except Exception:
