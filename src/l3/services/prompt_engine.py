...
class ContextAssembler:
    ...
    def add_lsp_diagnostics(self, file_path: str = "") -> int:
        """Add diagnostics context from LSP (pyright first, then AST fallback)."""
        try:
            ...
            issues = analyzer.diagnostics(file_path)
            if issues:
                errors = sum(1 for d in issues if str(d.get("severity", "")).lower() == "error")
                content = json.dumps(issues[:20], indent=1)
                item = ContextItem(
                    content=f"=== Diagnostics ===\n{content}",
                    source="lsp_diag",
                    priority=0.9 if errors else 0.3,
