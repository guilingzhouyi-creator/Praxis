...
class ModelService:
    ...
    def resolve_for_department(self, dept_type: str, overrides: dict | None = None) -> dict:
        """Resolve the model config for a department type (2.1-D4, config-driven).

        Maps the department type to its model-spec executor via
        ``department.model_role_for`` (review → "review", build/test →
        "build", otherwise "default") and resolves ``model_spec.{executor}``
        from praxis.yaml. The department → provider/model binding is
        **config data, never hardcoded**: deployments decide which
        department runs the strong model (e.g. review) and which runs the
        cheap one (e.g. build) by editing the ``model_spec`` section — and
        departments themselves are configurable (departments.yaml).

        Args:
            dept_type: One of DEPARTMENT_TYPES (general/build/test/review/custom).
            overrides: Optional per-call overrides (highest priority).

        Returns:
            Raw kwargs dict (model/max_tokens/temperature/reasoning_effort/
            thinking_budget); empty dict on any failure (degrade gracefully).
        """
        try:
            from l3.cell.department import model_role_for

            role = model_role_for(dept_type) or "default"
            return self.resolve_dict(role, overrides)
        except Exception as e:
            logger.debug("model_service: department resolve skipped: %s", e)
            return {}
