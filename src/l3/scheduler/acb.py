    ...
    def _notify(self, name: str, new: Any, old: Any) -> None:
        for cb in self._listeners.get(name, []):
            try:
                ...
                capture(f"acb handler {name} failed", error_code="E_ACB_HANDLER", component="acb", exc=e)
        ...
        for cb in self._global_listeners:
            try:
                ...
                capture(f"acb global handler failed for {name}", error_code="E_ACB_GLOBAL", component="acb", exc=e)
                logger.warning("acb handler: %s", e)
