...
def cardwrite_handler(args: dict, agent_id: str = "") -> dict:
    ...
    try:
        ...
        try:
            ...
            pass
        # L3A-C secretary (Phase 4): record the collaborative card production
        # as a contribution — gated by the l3a.secretary.enabled switch.
        try:
            from .secretary import get_secretary

            sec = get_secretary()
            if sec.enabled():
                sec.contribute("card", success=True, card_id=card.id, detail=title)
        except Exception:
            pass
        return {
            "success": True,
