...
class Session(
    ...
    def create(
        ...
        ``memory_scope`` binds the session to an isolated R1-R3 memory
        instance (e.g. "l3a" default, or "l3a-c-<n>" for an evolved
        secretary's own domain) — extension-first scope binding.
        ``cell_id``/``role`` carry the session's identity so AgentLoop
        inherits it and the identity binding injects the role fragment.
        """
