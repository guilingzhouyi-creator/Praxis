...
if TYPE_CHECKING:
    from .session import Session  # noqa: F401  (type annotation only)
...
class SessionPersistMixin:
    ...
    _ask: Any
    _lock: Any
    _on_card_completed: Any
    _subscribed_cards: Any
    created_at: Any
    ...
    def resume_from_archive(
        ...
        try:
            from l3.memory.central_memory import get_memory as _gm
            from l3.memory.memory_graph import get_graph as _gg

            g = _gg()
            if g.enabled:
                mem = _gm(inst.memory_scope)
