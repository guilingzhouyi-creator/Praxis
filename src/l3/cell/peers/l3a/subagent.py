...
_L3A_SPECS: dict[str, dict] = {
    ...
    # L3A-C secretary (Phase 4): analysis/reporting + collaborative card
    # production. "secretary" = assist mode (subordinate); "secretary-peer"
    # = peer mode (egalitarian) after the capability threshold is reached.
    "secretary": {
        "allowed_tools": ["read_file", "grep_search", "list_dir", "glob", "cardwrite"],
        "max_steps": _p.SA_SECRETARY_MAX_STEPS,
        "timeout": _p.SA_SECRETARY_TIMEOUT,
        "expect_keys": ["analysis", "report", "produced_cards"],
        "strategy": "balanced",
    },
    "secretary-peer": {
        "allowed_tools": ["read_file", "grep_search", "list_dir", "glob", "cardwrite"],
        "max_steps": _p.SA_SECRETARY_PEER_MAX_STEPS,
        "timeout": _p.SA_SECRETARY_PEER_TIMEOUT,
        "expect_keys": ["analysis", "report", "produced_cards", "decision"],
        "strategy": "balanced",
    },
}
...
class L3ASubAgentPool:
    ...
    def __init__(self, max_workers: int = 0):
        max_workers = max_workers or _p.SA_MAX_WORKERS
    ...
    def max_workers(self) -> int:
        """Return the current executor worker count."""
        return self._max_workers

    def set_max_workers(self, n: int) -> dict:
        """Resize the executor to n workers (elastic pool, D3).

        Rebuilds the executor: in-flight tasks finish on the old executor
        (non-blocking shutdown), new commissions go to the new pool.
        """
        n = max(_p.SA_MAX_WORKERS_MIN, min(int(n), _p.SA_MAX_WORKERS_CAP))
        with self._lock:
            if n == self._max_workers:
                return {"success": True, "max_workers": n, "resized": False}
            old = self._executor
            self._executor = ThreadPoolExecutor(max_workers=n, thread_name_prefix="l3a-sa")
            self._max_workers = n
        import contextlib

        with contextlib.suppress(Exception):
            old.shutdown(wait=False)
        logger.debug("l3a subagent pool: resized -> %d", n)
        return {"success": True, "max_workers": n, "resized": True}

    def resize_for_intensity(self, session_count: int) -> dict:
        """Adapt the pool to task intensity (D3): more active sessions →
        more workers, bounded by params caps."""
        target = max(
            _p.SA_MAX_WORKERS_MIN,
            min(
                _p.SA_MAX_WORKERS_CAP,
                (int(session_count) + _p.SA_INTENSITY_PER_WORKER - 1) // _p.SA_INTENSITY_PER_WORKER,
            ),
        )
        return self.set_max_workers(target)
    ...
    def _run(self, task_id: str, spec_name: str, task_text: str, strategy: str = "") -> dict:
        ...
        try:
            ...
            loop = AgentLoop(
                ...
                agent_id=agent_id,
                role="l3a_subagent",
                system=get_prompt("l3a.subagent.system").format(
                    spec_name=spec_name, allowed_tools=", ".join(allowed_tools)
                ),
...
def decision_bodies_for_intensity(intensity: int) -> int:
    """Evolve the decision-body count from task intensity (D3).

    The L3 decision layer is not a single decider: beyond the evolution
    threshold each additional threshold-worth of intensity evolves another
    secretary/decision body (each owning its own ``l3a-c-<n>`` memory
    scope). Returns at least 1 (the base secretary).

    Args:
        intensity: Task intensity signal (e.g. active sessions / queue depth).

    Returns:
        The number of decision bodies the layer should have.
    """
    threshold = getattr(_p, "SA_EVOLVE_SECRETARY_THRESHOLD", 6)
    if intensity < 1:
        return 1
    return max(1, int(intensity) // threshold + 1)
...
def l3a_spawn_handler(args: dict, agent_id: str = "") -> dict:
    ...
    # L3A-C secretary (Phase 4): when the requested spec is the assist-mode
    # secretary and the switch is on, commission with the mode-appropriate
    # spec — the peer spec once the capability threshold is reached.
    if spec == _p.L3AC_SPEC_ASSIST:
        try:
            from .secretary import get_secretary

            sec = get_secretary()
            if sec.enabled():
                spec = sec.commission_spec()
        except Exception as e:
            logger.warning("l3a secretary spec resolution failed: %s", e)
