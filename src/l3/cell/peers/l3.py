...
class CentralController:
    ...
    def register_cell(self, cell_id: str, territory: list[str], agents: list[str] | None = None) -> None:
        ...
        if len(self._cells) >= 2:
            ...
            # Phase 3 M3: per-Cell Agent handbooks activate at 2+ Cells —
            # generate the Cell-{cell_id}-Agents.md department brief for the
            # newly registered Cell (config-driven, never blocks boot).
            try:
                from l3.cell.peers.l3a.agents_md import generate_agents_md
                from l3.memory.memory_supply_chain import agent_md_active

                if agent_md_active(len(self._cells)):
                    r = generate_agents_md(cell_id=cell_id)
                    if not r.get("success"):
                        logger.warning("agents-md auto-activate failed for %s: %s", cell_id, r.get("error", "?"))
            except Exception as e:
                logger.debug("agents-md auto-activate skipped: %s", e)
        else:
    ...
    def _process_taskcard_result(self, card: TaskCard) -> dict:
                ...
                for shard in shards:
                    ...
                    for composite in self.b.composites:
                        if composite.next_cell == shard_cell:
                            ...
                            # HTN-B: decompose the shard subtask into a row
                            # plan and attach it to the dispatched card, so
                            # downstream execution can consume the check-prev
                            # / route steps. dispatch_to_next performs the
                            # handoff either way.
                            try:
                                subtask = {"id": f"shard-{shard_cell}", "name": shard_cell, "domain": domain}
                                plan = composite.route_subtask(subtask, "")
                                if plan:
                                    card_data["htn_b_plan"] = plan
                            except Exception as e:
                                logger.warning("HTN-B route_subtask failed for %s: %s", shard_cell, e)
                            composite.dispatch_to_next(card_data)
                            break
        ...
        # Department division (Phase 3): when the switch is on and the Cell
        # count reaches the threshold, record where content of this type is
        # directed — the org visibly splits into departments at routing time.
        try:
            from l3.cell.department import get_department_manager

            routed = get_department_manager().route_content(domain)
            if routed:
                result["departments"] = routed
        except Exception as e:
            logger.warning("department routing failed: %s", e)
