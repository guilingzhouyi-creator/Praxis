    ...
    for sl in slices:
        ...
        if type(sub_card).__name__ == "Card" and not hasattr(sub_card, "action"):
            ...
            sub_card = TerminalCard(
                ...
                params={
                    "phases": getattr(sub_card, "phases", []),
                    "intent": sub_card.intent or "",
                    # Card→identity linkage: carry the slice's structured domain
                    # so the per-slice dispatch scopes identity channels to it.
                    "_card_domain": getattr(sub_card, "domain", "") or "",
                },
            )
