...
from .config_handlers import (
    ...
    cfg_devices,
    cfg_diff,
    cfg_engineering_debug,
    cfg_gatechain,
    cfg_harness,
    cfg_htn,
    cfg_identity_binding,
    cfg_kernel,
    cfg_l3a,
    cfg_language,
    cfg_llm,
...
# Config-file watcher state (module-level so watch/stop are idempotent).
...
def load(config_path: str | None = None) -> dict:
    ...
    Self-healing: when no config file exists, provisions the default config
    from the shipped templates (ensure_config) and retries once — a deleted
    or never-created config_dir is rebuilt instead of failing.
    """
    try:
        import yaml
    except ImportError:
    ...
    if not path or not os.path.exists(path):
        from l3.config.ensure_config import ensure_config

        provision = ensure_config(path or "")
...
_builtin_handlers = [
    ...
    ("harness", cfg_harness),
    ("engineering_debug", cfg_engineering_debug),
    ("identity_binding", cfg_identity_binding),
    ("l3a", cfg_l3a),
    ("skill", cfg_skill),
    ("diff", cfg_diff),
...
def watch_config(interval: float = CONFIG_WATCH_INTERVAL, callback: Callable | None = None) -> dict:
    ...
    if _watch_thread is not None and _watch_thread.is_alive():
        return {"success": True, "path": path, "interval": interval, "already_running": True}
    ...
    _watch_thread = threading.Thread(target=_watcher, daemon=True)
    _watch_thread.start()
    return {"success": True, "path": path, "interval": interval}


def stop_watch_config() -> dict:
    """Stop the config-file watcher thread (idempotent)."""
    _watch_stop.set()
    return {"success": True, "stopped": True}


def reload_config() -> dict:
    """Force a one-shot reload of praxis.yaml (hot reload entry point)."""
    path = find_config()
    if not path:
        return {"success": False, "error": "no config file found"}
    return load_and_apply(path)
