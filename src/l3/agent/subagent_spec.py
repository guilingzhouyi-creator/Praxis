...
# Config-driven via config/discovery/subagent_specs.yaml (legacy: commands.yaml section).
...
def load_specs() -> dict[str, SubAgentSpec]:
    """Load subagent specs from YAML overrides, falling back to built-in defaults.

    Primary source: config/discovery/subagent_specs.yaml; legacy fallback:
    the commands.yaml ``subagent_specs:`` section. The section overrides
    individual specs by name; specs not present in YAML retain their
    ...
    # Apply YAML overrides — first existing source wins
    from l1.kernel.params.system import COMMANDS_CONFIG_PATH

    candidates = [
        os.path.join(os.getcwd(), "config", "discovery", "subagent_specs.yaml"),
        os.path.join(os.getcwd(), COMMANDS_CONFIG_PATH),
    ]
    for yaml_path in candidates:
        try:
            ...
            continue
        yaml_specs = cfg.get("subagent_specs", {}) if isinstance(cfg, dict) else {}
