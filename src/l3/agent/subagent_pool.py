    ...
    def _start_cleanup(self) -> None:
        ...
        def _cleanup_loop():
            while True:
                ...
                now = time.time()
                # Snapshot under the lock, compute expiry outside it, delete
                # under it — keeps the lock hold short (the scan itself is
                # O(n) and unavoidable, but the per-entry expiry math and
                # list build no longer run while holding the lock).
                with self._lock:
                    items = list(self._session_history.items())
                expired = [sid for sid, t in items if t.completed_at and now - t.completed_at > t.ttl]
                if expired:
                    with self._lock:
                        for sid in expired:
