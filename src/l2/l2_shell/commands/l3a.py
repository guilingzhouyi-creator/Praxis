...
from l2.bridge import l3a_dispatch as _l3a_dispatch
...
def _ensure() -> None:
    ...
    if not _l3a_initialized:
        from l2.bridge import l3a_start
...
def _cmd_session_monitor(args: list[str], session=None) -> dict:
    ...
    """
    from l2.bridge import session_monitor

    sub = args[0].lower() if args else ""
    if sub in ("on", "off"):
        return session_monitor(enabled=sub == "on")
...
def _cmd_session_reload(args: list[str], session=None) -> dict:
    ...
    """
    from l2.bridge import auto_reload_session
...
def _cmd_session_history(args: list[str], session=None) -> dict:
    ...
    for arg in args:
        if arg.lower() in ("on", "off"):
            return set_session_history(enabled=arg.lower() == "on")
        if arg.startswith("limit=") and arg[6:].isdigit():
            limit = int(arg[6:])
        elif arg.startswith("session="):
            session_id = arg[len("session=") :]
    return {"status": session_history_status(), **query_session_history(limit=limit, session_id=session_id)}
