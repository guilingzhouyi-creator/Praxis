...
def bump(version: str, dry_run: bool) -> list[str]:
    ...
    # 5. CHANGELOG.md: move [Unreleased] into the versioned section, reopen empty.
    _migrate_changelog(version, dry_run, changes)

    return changes


def _migrate_changelog(version: str, dry_run: bool, changes: list[str]) -> None:
    """Move the [Unreleased] block into a versioned section and reopen an empty one."""
    text = CHANGELOG.read_text(encoding="utf-8")
    pattern = re.compile(r"^## \[Unreleased\](.*?)(?=^## \[|\Z)", re.MULTILINE | re.DOTALL)
    m = pattern.search(text)
    if not m:
        raise RuntimeError("expected a [Unreleased] block in CHANGELOG.md")
    today = datetime.date.today().isoformat()
    replacement = f"## [{version}] - {today}{m.group(1)}\n\n## [Unreleased]\n\n"
    new_text, n = pattern.subn(replacement, text)
    if n != 1:
        raise RuntimeError("expected exactly one [Unreleased] block in CHANGELOG.md")
    if not dry_run:
        CHANGELOG.write_text(new_text, encoding="utf-8")
    changes.append(f"CHANGELOG.md: [Unreleased] -> [{version} - {today}]")
