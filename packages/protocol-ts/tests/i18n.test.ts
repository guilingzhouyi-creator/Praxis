...
describe("I18n", () => {
  it("defaults to en with the standard available locales", () => {
    const i18n = new I18n();
    expect(i18n.getLocale()).toBe("en");
    expect(i18n.getAvailableLocales()).toEqual(["en", "ja", "ko", "zh-CN"]);
  });

  it("translates dot-notation keys and substitutes kwargs", () => {
    const i18n = new I18n();
  ...
  it("switches locale, rejecting unknown codes with an en fallback", () => {
    const i18n = new I18n();
...
describe("lang builtin", () => {
  ...
  it("reports current locale and available locales", () => {
    const { dispatcher } = setup();
    const out = dispatcher.dispatch({ name: "lang", args: [] }, { sessionId: "s-1" });
    expect(out).toEqual({
      kind: "local",
  ...
  it("switches locale with an argument and reports the previous one", () => {
    const { dispatcher } = setup();
    const out = dispatcher.dispatch({ name: "lang", args: ["ja"] }, { sessionId: "s-1" });
    expect(out).toEqual({
      kind: "local",
      data: { lang: "ja", previous: "en", available: ["en", "ja", "ko", "zh-CN"] },
    });
    // The switch persisted on the shared I18n instance.
    const again = dispatcher.dispatch({ name: "lang", args: [] }, { sessionId: "s-1" });
    expect(again).toMatchObject({ data: { lang: "ja" } });
  });

  it("degrades to the plain en reply when no i18n is injected", () => {
    const dispatcher = new Dispatcher();
