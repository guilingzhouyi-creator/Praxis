//! Emit a release-mode fixed-work WorkerPool evidence report.

use l1_kernel_rs::benchmark::{BenchmarkEvidence, BenchmarkMetadata, FixedWorkSpec};
use l1_kernel_rs::benchmark_runner::run_worker_pool_batch;
...
fn main() {
    let spec = FixedWorkSpec::new("worker.pool.batch", 4_096, vec![1, 2, 4], 3)
        .expect("benchmark specification is valid");
    let report = run_worker_pool_batch(spec, 4_096).expect("worker pool benchmark completed");
