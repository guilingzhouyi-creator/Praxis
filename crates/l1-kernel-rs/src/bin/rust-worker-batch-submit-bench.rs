//! Emit a release-mode fixed-work WorkerPool batch-submit evidence report.

use l1_kernel_rs::benchmark::{BenchmarkEvidence, BenchmarkMetadata, FixedWorkSpec};
use l1_kernel_rs::benchmark_runner::run_worker_pool_batch_submit;
...
fn main() {
    let spec = FixedWorkSpec::new("worker.pool.batch_submit", 4_096, vec![1, 2, 4], 3)
        .expect("benchmark specification is valid");
    let report =
        run_worker_pool_batch_submit(spec, 4_096, 32).expect("batch-submit benchmark completed");
