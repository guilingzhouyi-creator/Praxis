//! Emit fixed-work bounded AgentLoopBook snapshot-page evidence.

use l1_kernel_rs::benchmark::{BenchmarkEvidence, BenchmarkMetadata, FixedWorkSpec};
use l1_kernel_rs::benchmark_runner::run_agent_loop_book_snapshot_page;
...
fn main() {
    let spec = FixedWorkSpec::new("agent_loop.book.snapshot_page", 4_096, vec![1, 2, 4], 3)
        .expect("snapshot page benchmark specification is valid");
    let report = run_agent_loop_book_snapshot_page(spec, 4_096, 64)
