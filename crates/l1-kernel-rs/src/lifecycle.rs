...
mod tests {
    use super::{
        LifecycleErrorCode, LifecycleRecord, LifecycleRegistry, LifecycleState, reset_lifecycle,
        state,
    };
    ...
    fn crash_recovery_and_shutdown_paths_are_explicit() {
        ...
        lifecycle.restore(record).unwrap();
    ...
    fn malformed_checkpoint_is_structured() {
        let lifecycle = LifecycleRegistry::new();
        let error = lifecycle.restore_encoded(b"not-json").unwrap_err();
    ...
    fn global_lifecycle_can_be_reset() {
        ...
        assert!(super::transition(LifecycleState::Booting));
        reset_lifecycle();
        assert_eq!(state(), LifecycleState::Halted);
    }
    ...
    fn shared_lifecycle_vectors_match_python_reference() {
        let vectors = fixture();
        for path in vectors["paths"].as_array().unwrap() {
            let lifecycle = LifecycleRegistry::new();
            let states = path["states"].as_array().unwrap();
            for value in states.iter().skip(1) {
                let target = LifecycleState::parse(value.as_str().unwrap()).unwrap();
                assert!(lifecycle.transition(target));
            }
            let expected = LifecycleState::parse(states.last().unwrap().as_str().unwrap()).unwrap();
            assert_eq!(lifecycle.state(), expected);
        }

        for invalid in vectors["invalid_transitions"].as_array().unwrap() {
            let lifecycle = LifecycleRegistry::new();
            let source = LifecycleState::parse(invalid["from"].as_str().unwrap()).unwrap();
            let target = LifecycleState::parse(invalid["to"].as_str().unwrap()).unwrap();
            match source {
        ...
        for case in vectors["install_decisions"].as_array().unwrap() {
            let record: LifecycleRecord = serde_json::from_value(case["record"].clone()).unwrap();
            let lifecycle = LifecycleRegistry::from_record(record).unwrap();
            assert_eq!(
                lifecycle.should_install(case["current_schema"].as_str().unwrap()),
                case["should_install"].as_bool().unwrap()
            );
        }
    }
}
