...
mod tests {
    ...
    fn message_defaults_and_wire_round_trip_are_stable() {
        ...
        let restored: LockMessage = serde_json::from_value(wire).unwrap();
        assert_eq!(restored, message);

        let defaults: LockMessage =
            serde_json::from_value(json!({"op": "ACQUIRE", "lock_name": "lock:defaults"})).unwrap();
    ...
    fn request_is_woken_by_response_and_timeout_cleans_pending_message() {
        ...
        assert_eq!(waiter.join().unwrap(), json!({"granted": true}));
