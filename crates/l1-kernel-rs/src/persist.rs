...
#[cfg(test)]
mod tests {
    use super::{EventStore, PERSIST_QUERY_LIMIT};
    use serde_json::json;
    use std::fs;
    ...
    fn append_query_count_and_last_seq_preserve_python_shape() {
        ...
        let store = EventStore::with_commit_batch(&path, 2).unwrap();
        assert_eq!(store.last_seq().unwrap(), 0);
        assert_eq!(store.append("alpha", Some(json!({"v": 1}))).unwrap(), 1);
        assert_eq!(store.append("beta", Some(json!({"v": 2}))).unwrap(), 2);
        assert_eq!(store.append("alpha", Some(json!({"v": 3}))).unwrap(), 3);
        assert_eq!(store.count(None).unwrap(), 3);
        assert_eq!(store.count(Some("alpha")).unwrap(), 2);
        let rows = store.query(Some("alpha"), 1, PERSIST_QUERY_LIMIT).unwrap();
    ...
    fn append_many_is_sequenced_and_empty_payload_is_an_object() {
        ...
        assert_eq!(store.append("empty", None).unwrap(), 3);
        assert_eq!(store.query(None, 0, 10).unwrap()[2].payload, json!({}));
        cleanup(&path);
    }
    ...
    fn records_survive_reopen_and_sequence_gaps_fail_closed() {
        ...
        assert_eq!(reopened.last_seq().unwrap(), 1);
        assert_eq!(reopened.query(None, 0, 10).unwrap()[0].event, "persisted");
        fs::write(
            &path,
