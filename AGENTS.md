...
- L5→L4→L3→L2→L1 only; L1 cannot import upper layers; 95 pre-existing cross-layer imports allowlisted
...
Hook systems: `.githooks/` (bash: pre-commit/commit-msg/post-checkout — load-bearing governance gates) vs `.pre-commit-config.yaml` (style lint only, not governance).
...
- `pyproject.toml` sets `addopts = "-n auto --dist loadfile"` for parallel pytest; `-n 0` pins in CI are explicit overrides
...
`src/l3/cell/peers/l3a/` — L3A session system (24 modules) | `src/l3/cell/peers/l3.py` — CentralController
