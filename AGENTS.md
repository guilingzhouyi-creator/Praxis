...
`make doc-stats` (`scripts/py/gen-doc-stats.py`) when they drift; never
...
src/l2/ — Shell family: shells/ dialect adapters (TerminalShell, ShellFamily registry, per-session ShellSession)
  over the shared l2_shell/ engine (49 YAML commands + code-registered `_cmd_*` handlers), i18n, agent selector
...
src/l1/kernel/params/ — runtime constants (8 modules); exact counts via `make doc-stats` —
  generated by `scripts/py/gen-doc-stats.py`, drift-gated by `scripts/py/check-doc-stats.py`;
  never hand-edit
src/l1/kernel/ports/ — 12 `*Port(ABC)` abstractions (package: `core.py`/`service.py`/
...
(`scripts/py/gen-doc-stats.py`); `scripts/py/check-doc-stats.py` drift-gates
...
- **Version bumps are atomic**: `pyproject.toml` + AGENTS.md header + `KERNEL_VERSION` + its tests + `docs/` SOC references in ONE commit — `python scripts/py/bump-version.py <ver>` (or `make bump-version`; `--dry-run` previews).
...
| Path | Description |
|------|-------------|
| `release/` | Release/packaging output placeholder (empty; populated by release tooling, see `scripts/py/bump-version.py`) |
