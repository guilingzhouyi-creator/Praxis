...
Architecture guide for the L2 shell family (`src/l2/`). Linux-model design: one stable engine contract (`l2_shell/` dispatch), many shell dialects (`shells/`) translating frontend dialects into it. A new frontend registers a new shell — the engine never changes.
...
- **Handlers are the frontend contract**: CLI/TUI/API render the same dict-returning surfaces. Handlers live in `src/l2/l2_shell/commands/*.py` (YAML: `config/commands.yaml` — 51 commands; code-registered `_cmd_*` handlers).
