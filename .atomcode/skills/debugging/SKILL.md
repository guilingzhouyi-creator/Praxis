...
- **Gate rejection**: a push rejected by `push-both.sh main` is the mainline net-delta gate (`verify-main-merge-gate.sh`) — the change is below the ≥ 1000 net threshold (or comment/deletion locks tripped). Do NOT self-waive with `MERGE_GATE_SKIP=1`; accumulate on the worktree branch or ask the user.
