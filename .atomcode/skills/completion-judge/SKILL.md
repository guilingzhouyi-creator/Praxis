...
- Run: `bash scripts/sh/verify-completion.sh`
  (WSL: `wsl -d Ubuntu -- bash -c "cd /home/guiling/dev/praxis && bash scripts/sh/verify-completion.sh"`)
...
| 3 | Net code delta | `verify-main-merge-gate.sh` (three locks) |
...
| 9 | Singleton drift | `scan_singletons.py` vs `_RESETS` |
