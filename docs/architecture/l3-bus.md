...
L1 calls are best-effort and never break the protected path. The same sink
feeds the security evidence chain (second consumer): every
`security.gate.g4.*` / `security.gate.skill_use.*` metric also records an
evidence point via `record_from_metric()` (see
[security-evidence.md](security-evidence.md)). L1-originated policy flips
(`security_policy_change`, emitted by the skill manager) are additionally
observed through an idempotent event-bus listener in the evidence module —
the L1 → L3 channel is metric/event-only, never an L3 import.
