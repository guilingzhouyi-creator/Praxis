...
with the lifetime of the archive.

`CandidateLedgerPort` is the language-neutral adapter seam for listing,
validation, lifecycle transitions, and policy state. Candidate records use a
stable JSON-shaped schema (`id`, `fingerprint`, `binding`, `evidence`,
`validation`, `state`, and timestamps); lifecycle state names and limits come
from `params/` and remain the single source of truth. Publication intent and
LLM orchestration stay in L3 above the port. A future Rust ledger can therefore
replace indexed storage and transition checks through the same port while the
Python L3 orchestration, API, L2 shell, and callers remain unchanged. The
Rust adapter must preserve atomic transition semantics, bounded evidence, and
