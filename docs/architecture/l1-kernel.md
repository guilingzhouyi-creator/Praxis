...
Persistent open validates this configuration root before applying an unclean
state recovery transition. A foreign or otherwise invalid configuration root
therefore fails without advancing the `StateStore` recovery generation or
mutating its lifecycle record.
