...
worker join list remain mutex-owned. The independent `tests/core/kernel_test_worker.rs` target
...
`systems/rust-kernel-engine/l1-kernel-rs/tests/session/kernel_test_agent_loop.rs` target and does not grant runtime
...
The independent `tests/runtime/kernel_test_agent_loop_execution.rs` target covers success,
...
If the later clean `StateStore` commit fails after the execution document was
written, shutdown immediately demotes that execution document to unclean and
best-effort marks the state root crashed before returning the state-store
error. This prevents a clean execution checkpoint from being paired with a
failed lifecycle commit during the next reopen.
...
checkpoint before applying an unclean state recovery transition. A foreign or
otherwise invalid attached root therefore fails without advancing the
`StateStore` recovery generation or mutating its lifecycle record.
...
`HostInputActivityPort` provides the first T4b mechanism seam. It accepts an
`InputActivityHostAdapter` that reports explicit `Granted`, `Denied`, or
`Unavailable` permission and returns a caller-timed aggregate sample. The port
delegates reduction to `InputActivityProbe`, exposes permission failures as an
...
default. The independent `tests/runtime/kernel_test_entry.rs` target covers fresh, clean,
...
`tests/protocol/kernel_test_protocol_host.rs` target.
...
Host command registration now also populates the router's GateChain whitelist.
Every registered non-system command is adjudicated with the trusted context
ring before capability execution; a blocked verdict is returned as the normal
R7 denial envelope and never reaches the executor. Wire metadata can provide
only the existing ring input for non-strict adapters; a bound host context
always wins, and no wire approval field is accepted. This closes the previous
ordinary-command bypass while keeping settings authorization and process
admission as separate explicit seams. Coverage is in
...
AgentLoop work; its behavior is covered by independent `tests/session/kernel_test_session.rs` and
...
independent `tests/session/kernel_test_session_store.rs` target.
...
these safe metadata states. Coverage is isolated in `tests/session/kernel_test_execution_store.rs`;
...
`systems/rust-kernel-engine/l1-kernel-rs/tests/core/kernel_test_sync.rs`; the shared RWLock vectors remain in
...
queries. Rust admission now rejects blank, NUL-containing, overlong section
...
I/O provider discovery, and Constitution policy remain outside the candidate.

The Rust `constitution_io` candidate now owns a narrow, Rust-native document
boundary for territory and GateChain values. `TerritoryConstitution` parses
known scalar fields strictly, renders sorted deterministic Markdown, validates
territory identities, and computes stable territory diffs. `ConstitutionStore`
keeps an in-memory snapshot aligned with an atomically replaced, flushed file;
failed writes leave both the prior snapshot and temporary-file directory
unchanged. Temporary siblings use exclusive creation and the parent directory
is synced after rename where the host supports directory syncing. Source,
GateChain keys, and selected paths reject embedded NULs before mutation;
concurrent updates are serialized per store. This is a filesystem-bearing R4
candidate only: SettingsCenter discovery, provider
selection, prompt injection, EventBus effects, and production Constitution
authority remain outside the module. Independent coverage lives in
`systems/rust-kernel-engine/l1-kernel-rs/tests/policy/kernel_test_constitution_io.rs`.
...
`systems/rust-kernel-engine/l1-kernel-rs/tests/process/kernel_test_process_adapter.rs` target, and
...
explicit terminate/reap. `tests/process/kernel_test_managed_process.rs` is the independent public
...
sharing one table. `tests/process/kernel_test_process_bridge.rs` and the separate
...
`tests/process/kernel_test_process_group_runtime.rs` target covers bounded budgets, cancellation,
...
The ProcessTable-authoritative path also exposes `spawn_constrained` and
`spawn_gated_constrained`. It reuses the explicit Agent process policy and
G1-G5 capability request before calling the bridge, preserving the rule that
gate or constraint rejection creates no host child and no table binding. The
gate ledger remains the source of admission evidence; PTY discovery and
platform signal policy remain host-owned.

The runtime accepts a caller-owned `AuditLog` through `new_with_audit` and
exposes it through `audit()`. Group creation, gate/constraint admission,
bridge failures, successful spawn, and stop requests record bounded metadata
for the same execution path. The audit detail excludes argv, environment
values, and host process identifiers; journal durability and retention remain
the host's explicit `AuditLog`/`EventStore` configuration.
...
The independent `tests/terminal/kernel_test_terminal_probe.rs` target covers deterministic
...
injected invocation prefix. The independent `tests/process/kernel_test_process_constraints.rs`
...
### Rust skill registry and staged guidance

The Rust `skill::SkillRegistry` is a mechanism-only clean-break mapping of the
bounded L1 portion of Python `SkillManager`. It owns typed metadata, lifecycle
status, productive/offensive posture, progressive disclosure, agent/Cell/global
scope, bounded hard/soft dependencies, write authorization, builtin read-only
protection, Cell bindings, usage counters, deterministic keyword retrieval,
staged guidance, card-completion advancement, guidance-DAG cycle reporting,
`/skills` virtual listing, and versioned in-memory checkpoint/restore. Content
is truncated and validated before admission; identity-less external writes and
unsafe content fail closed.

The candidate deliberately excludes Markdown/YAML discovery, prompt providers,
EventBus effects, R4 distillation/DPO, Card/TODO/AgentLoop policy execution,
L3 API/L2Shell routing, and Python-state import. `tests/fixtures/kernel_skill_vectors.json`
freezes only the retained authorization/posture/checkpoint values, while the
independent `tests/policy/kernel_test_skill.rs` target covers bounded mutation,
scope/audience filtering, staged progression, graph validation, checkpoint
determinism, and concurrent access. This is a Rust-native mechanism slice, not
a production skill authority or a Python compatibility layer.

### Terminal-backed AgentLoop composition

`agent_loop_terminal::AgentLoopTerminalBridge` is the explicit R4 composition
seam between the opaque `TerminalBook` mailbox and the Rust-owned
`AgentLoopExecutionBridge`. It validates the loop/session/terminal identity
triple and both running states before admitting I/O. Terminal input remains a
caller-owned `TerminalFrame`; an injected decoder converts it to `SessionInput`
under panic containment, and the existing execution bridge remains responsible
for runtime reservation, session sequencing, caller-owned provider/tool action,
and structured failure. Grouped input decodes every frame before queue
admission and applies a fixed batch bound, so decoder or correlation failures
cannot partially enqueue work.

The same bridge publishes opaque output/error frames only to the validated
terminal and rejects input-stream misuse or oversized frames before mailbox
mutation. Dequeue/retry policy stays with the transport adapter: callers
normally obtain frames through `TerminalBook::take_input`, then transfer the
frame to the bridge. No encoding, shell default, PATH lookup, PTY creation,
provider execution, hardware input, or production boot authority crosses this
boundary. The independent
`tests/session/kernel_test_agent_loop_terminal.rs` target is the evidence
slice; future TS/L2 code should consume this versioned correlation and frame
contract rather than mirror Python classes.
