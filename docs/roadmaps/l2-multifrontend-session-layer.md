...
| **P1 边界迁移** | 建 L3 command bridge；65 个 `_cmd_*` 控制命令改走桥；selector 策略迁 L3；配置权威收敛（settings 单一写面）；`_cmd_history` 等桩补齐为真实现；i18n 全面收编 ✅（2026-08-20：47 处 f-string error 串全部改走 `shell.app_error.*` key（31 个，含 {var} 插值，4 locale），`test_i18n_l2_regression` 正则已补 f-string 盲区） | L2→L3 直连导入清零（allowlist 移除）；策略写操作全部经桥 |
