...
| E5 | 三张进程/终端表并存：L1 `ProcessTable`（PCB，逻辑进程权威）、L3 `_terminals`（AgentTerminal 注册表，运行态权威）、L2 `TerminalManager`（`subprocess.Popen` + SIGTERM/SIGKILL，**全库 0 调用方，死代码**） | `src/l1/kernel/process.py:184`、`src/l3/agent_terminal/__init__.py:564`、`src/l2/shell_session.py:73-137` |
...
| 阶段 | 内容 | 验收 |
|---|---|---|
| **P0 止血（当前）** | 合入 WT 的 `invoke_gated` 单门 + G1 fail-closed；修复 `_l3a_intent`（改走 `l3.cell.peers.l3` 或经桥）；删除 `shell_session.py` 死代码 | 门禁测试绿；默认 L3A 意图路径可用；L2 无 Popen |
...
| **P2 协议 v1** | 统一会话数据层落地：envelope/kind/seq/ack、outbox + recovery、event projection、multiplexing；web 端点迁移到协议 | 双前端（web + TUI）同会话并发可恢复；断线重放无丢失 |
