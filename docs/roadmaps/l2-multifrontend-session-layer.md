...
| **P2 协议 v1** | Python 参考实现已落地：`src/l2/protocol/`（envelope/schema/records/host）+ 契约钉；只读 TS parity mirror 已落地：`packages/protocol-ts/`（共享 fixture + Vitest）；web 端点已双模式接入协议（2026-08-20：`/api/v2/shell` 检测 envelope 走共享 ProtocolHost，旧 dict 兼容）；剩余：event projection、multiplexing | 双前端（web + TUI）同会话并发可恢复；断线重放无丢失；TS 镜像测试与 Python 契约钉同绿；TS 仍不拥有运行时状态 |
