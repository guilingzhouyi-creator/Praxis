...
| `terminal-shell.ts` | 🟡 `TerminalShell` 方言运行面（builtins/history/bridge/L3A/Direct tool）；REPL 输入循环与渲染仍由前端负责 |
...
1. **P3 收尾**：terminal dialect/session 已落地；剩余真实 SSH 端点接入
   （远端 stdio host 已通，按需）+ 五前端矩阵真实接入 + REPL 渲染终态。
