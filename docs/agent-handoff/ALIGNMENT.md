...
| 2026-08-25 | scripts/sh/verify-completion.sh + judge-stats.sh + docs/architecture/completion-judge.md | AtomCode | judge scorecard hardening: zero-value metric preservation, full-mode rate tracking, git exemption parsing (-z), flock concurrency, worktree log resolution | in feature/judge-stats-hardening (this commit) |
