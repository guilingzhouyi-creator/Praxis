...
tests** and **script subprocesses** — both Python3-specific patterns that
...
These scan the entire source tree. Each is inherently Python3-specific.
...
These run `scripts/py/` via subprocess. The scripts are Python3-specific.
...
| `test_githooks_commit_msg` | 0.97s | Runs bash hook + Python3 detect_agent | Hook + Python3 script → TS lint rule |

## Python3-Specific Bindings (TS Rewrite Blockers)

### P0 — Must be addressed before TS rewrite

1. **`tests/conftest.py` singleton reset (`_RESETS` dict, 60+ entries)**
   - Python3-specific: module path as string key, dynamic function dispatch
...
2. **`tests/runner.py` — Python3 test runner**
   - Already improved, but still Python3
   - TS equivalent: vitest/jest + custom runner, or Node.js CLI

3. **`scripts/py/` — Python3 scripts (6 scripts)**
...
4. **`.githooks/commit-msg` — bash hook that runs Python3**
   - Calls `detect_agent.py` (Python3) and `commit_scan.py` (Python3)
   - TS rewrite: the hook must call a Node.js script instead of Python3

### P1 — Architecture patterns that must be redesigned

5. **`l1.kernel.ports` — ABC port system (16 ports)**
   - Python3-specific: ABC + typing + `get_process_port()` singleton
...
6. **`conftest.py` `importlib.reload(_cmds_mod)`**
   - Python3-only: reloads entire L2 command tree on every test
...
7. **`l1.kernel.params/` — 12 magic-number modules**
   - Python3 constants → TS `const` or config files
...
9. **`l1.kernel.prompts` — prompt templates as `_DEFAULTS` dict**
   - Python3 dict → TS template literals or separate `.md` files

10. **`l3.tool_system.tool_pipeline` — 9-step execution pipeline**
    - Architecture translates directly; implementation is Python3-specific

## Recommended Next Steps (ordered by TS-rewrite value)

### Step 1: Decouple script tests from Python3 runtime
...
- This eliminates the 2 largest remaining slow tests (7.30s + 6.19s) and
  removes the Python3-script dependency from the test suite

### Step 2: Decouple git hooks from Python3
...
- This removes the `test_githooks_commit_msg` 0.97s Python3 dependency
...
After each step:
- `python -m pytest tests -q -n auto` → green (until Python3 tests are replaced)
- TS-equivalent gate tests pass
- `make precommit` green (both Python3 and TS gates)

## Branch Strategy

- Continue on `feature/test-perf-slicing` for Python3-side optimizations
- Create `feature/tests-ts-rewrite` for the TypeScript rewrite itself
- Merge order: data-driven snapshots → Python3 script decoupling → TS rewrite