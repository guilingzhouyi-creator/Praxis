"""User profile side-channel —grows a model of each user's preferences and
...
Architecture (side-channel, mirrors Mer/R5 philosophy):
    ...
    User interaction (cards, approvals, sessions, corrections, API ingest)
        • collectors (event bus + explicit ingest)
        ▼    ProfileStore (per-user typed entries, confidence, TTL, decay)
        • refiner (LLM synthesis →trait entries; rule fallback)
        ▼    Consumers (L3A cardwrite/prompt, any service via get_port("profile"))
        • R4 archive (fonds=user_profile, series=user_id) —portable
        ▼    export/import + /api/v2/profile/*
...
class ProfileStore:
    ...
    def snapshot(
        ...
        Returns {user_id, kinds, entries, updated_at} —kinds restrict the
...
class UserProfileService(BaseService):
    """User profile side-channel —collection, refinement, query, portability."""
    ...
    # ── Refinement (LLM synthesis with rule fallback) ──
...
# ── Singleton (conftest-resettable) + port self-registration ──
