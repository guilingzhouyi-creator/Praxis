...
class Cell(
    ...
    Usage:
      cell = Cell("cell-1", territory=["systems/python-reference-runtime", "docs"])
      cell.add_agent("agent-a", role="reader", territory=["docs"], ring=1)
      cell.add_agent("agent-b", role="writer", territory=["systems/python-reference-runtime"], ring=2)
    ...
    def __init__(
        ...
        # Cell extension registry (C1): optional named hooks (e.g. department
        # division) attach here instead of hardcoding into the Cell class.
        self._extensions: dict[str, Callable] = {}

    # ── Cell extension registry (C1) ──

    def register_extension(self, name: str, fn: Callable) -> bool:
        """Register a named Cell extension hook (idempotent by name).

        Extensions are invoked by ``run_extension(name, **kwargs)`` at
        lifecycle points (boot / card completion). Registration is
        config-driven — a deployment opts in via config/discovery/*.yaml
        rather than hardcoding extensions into the Cell class.
        """
        with self._lock:
    ...
    def run_extension(self, name: str, **kwargs: Any) -> dict:
        """Invoke a registered extension; returns its result or an error."""
        with self._lock:
            fn = self._extensions.get(name)
        if fn is None:
            return {"success": False, "error": f"extension '{name}' not registered"}
        try:
            result = fn(cell_id=self.cell_id, **kwargs)
            return {"success": True, "extension": name, "result": result}
        except Exception as e:
            logger.warning("cell extension '%s' failed: %s", name, e)
            return {"success": False, "error": str(e)}

    def list_extensions(self) -> list[str]:
        """Return registered extension names (sorted)."""
        with self._lock:
