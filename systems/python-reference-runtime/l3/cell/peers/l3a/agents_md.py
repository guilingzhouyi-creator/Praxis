...
# Layer table: relative systems/python-reference-runtime/ dir -> display name. Mirrors scripts/py/gen_doc_stats.py.
...
def _dir_stats(root: Path, rel: str) -> dict:
    """Return {"files": N, "lines": N} for systems/python-reference-runtime/<rel>, or zeroed dict when absent."""
...
def _find_project_root(start: str = "") -> str:
    """Locate the project root: the nearest ancestor of start holding systems/python-reference-runtime/.

    Falls back to start (or cwd) when no systems/python-reference-runtime/ ancestor is found, so the
    collector degrades gracefully outside a source checkout.
    """
    cur = Path(start or os.getcwd()).resolve()
    for _ in range(6):
        if (cur / "systems/python-reference-runtime").is_dir():
            return str(cur)
        if cur.parent == cur:
            break
        cur = cur.parent
...
def collect_project_info(project_root: str = "") -> dict:
    ...
    Args:
        project_root: Explicit project root; empty means auto-discover by
            walking up from cwd until a ``systems/python-reference-runtime/`` directory is found.
