...
MANAGED_OUTPUT_DIR: str = "l3a-outputs"
...
SA_MAX_WORKERS_MIN: int = 1
SA_MAX_WORKERS_CAP: int = 16
SA_INTENSITY_PER_WORKER: int = 3  # active sessions per worker (resize divisor)
SA_EVOLVE_SECRETARY_THRESHOLD: int = 6  # intensity at which decision bodies evolve
...
# ── L3A-C secretary (Phase 4) ──
# The secretary undertakes analysis/reporting and collaborative card
# production in "assist" mode (subordinate). Reaching the capability
# threshold upgrades it to "peer" mode (egalitarian collaboration), at
# which point L3A acts as an intent-decision hub rather than a single
# session manager.
L3AC_CAPABILITY_THRESHOLD: int = 3
L3AC_HISTORY_MAX: int = 1000
L3AC_ENABLED_DEFAULT: bool = True
L3AC_MODE_ASSIST: str = "assist"
L3AC_MODE_PEER: str = "peer"
L3AC_SPEC_ASSIST: str = "secretary"
L3AC_SPEC_PEER: str = "secretary-peer"
# Memory-scope prefix for evolved secretaries: an evolved secretary owns
# an isolated R1-R3 instance under "l3a-c-<n>" (extension-first, no
# hardcoded single scope); the default single secretary keeps "l3a".
L3AC_SCOPE_PREFIX: str = "l3a-c-"
L3AC_PEER_ROLE: str = "l3a-secretary"
SA_SECRETARY_MAX_STEPS: int = 8
SA_SECRETARY_TIMEOUT: float = 60.0
SA_SECRETARY_PEER_MAX_STEPS: int = 12
SA_SECRETARY_PEER_TIMEOUT: float = 90.0
