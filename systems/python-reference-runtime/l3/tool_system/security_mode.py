...
def set_security_mode(mode: str, confirmed: bool = False, source: str = "api") -> dict:
    ...
    if mode == SECURITY_MODE_TEST and not confirmed:
        ...
        try:
            from l3.tool_system.security_evidence import DECISION_WARN, record_evidence
    ...
    ingest_security_metric(
        ...
        value=1.0 if isinstance(posture, dict) and posture.get("full_power") else 0.0,
        tags={"mode": mode},
    )
    ingest_security_metric(
    ...
    # Evidence chain: attack mode opens an attack chain, productive closes it.
    try:
        ...
        if mode == SECURITY_MODE_TEST and confirmed:
            record_evidence(
                phase="posture",
                gate="security_mode",
                ...
                source=source,
                tags=tags,
                raw={"mode": mode, "source": source},
                chain_kind="attack",
            )
        elif mode == SECURITY_MODE_PRODUCTIVE:
            record_evidence(
                phase="posture",
                gate="security_mode",
                decision=DECISION_CHANGE,
                target="restore:productive",
                source=source,
                tags=tags,
                chain_kind="attack",
            )
            get_evidence().close_open(kind="attack")
    except Exception:
        pass
    return result
