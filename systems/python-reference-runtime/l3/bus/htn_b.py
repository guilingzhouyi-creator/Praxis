...
def _decompose_route_forward(
    ...
    """
    tid = root.id
    # 2.1-D2 wiring: the "check prev L2 cache" step reads the previous
    # cell's shared summary from the TieredCache L2 surface (HTN-B reads
    # only this tier, never successor cells' full state).
    try:
        from l3.memory.tiered_cache import get_tiered_cache

        prev_summary = get_tiered_cache().get_shared_summary(prev_cell, "build_summary") or {}
    except Exception as e:
        logger.debug("htn_b: L2 summary read skipped: %s", e)
        prev_summary = {}
    return [
        Task(
            id=f"{tid}-check-prev",
            ...
            domain=root.domain,
            agent_id=prev_cell,
            description=f"prev L2 build summary: {prev_summary}" if prev_summary else "prev L2 build summary: <none>",
            params={
                "action": "read_summary",
                "prev_cell": prev_cell,
                "query": root.name,
