...
class LogService(BaseService):
    ...
    def log(self, level: str, message: str, service: str = "", agent_id: str = "", task_id: str = "") -> dict:
        """Log an entry at an explicit level and return the write result.

        Public level-agnostic entry point for callers that carry a dynamic
        level (e.g. error_bus) — avoids reaching into the private ``_log``.
        """
        return self._log(level, message, service, agent_id, task_id)
