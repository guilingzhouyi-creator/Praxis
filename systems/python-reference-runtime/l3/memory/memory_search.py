...
def search_long_term(mem, query: str, agent_id: str | None = None, limit: int = 10) -> list[dict]:
    ...
    try:
        ...
        # Phase 3 M1: identity/Cell-domain gate on FTS retrieval.
        try:
            from .memory_domain_filter import get_memory_filter

            _f = get_memory_filter()
            if _f.status().get("enabled"):
                records = _f.filter_entries(records, cell_id="", role="", scope="")
        except Exception:
            logger.debug("memory_search: domain filter skipped")
        return records
    except Exception:
        return []
