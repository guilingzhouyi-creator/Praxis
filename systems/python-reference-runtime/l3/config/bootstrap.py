...
def default_config() -> dict:
    ...
    Template-first: reads the shipped ``config/praxis.yaml`` template from
    ``PraxisPaths.config_templates_dir`` when available (single source of
    truth for defaults); falls back to kernel-params-derived constants so a
    template-less deployment still gets a valid baseline.
    """
    from l1.kernel.paths import get_paths

    tpl = os.path.join(get_paths().config_templates_dir, "praxis.yaml")
    try:
        if os.path.isfile(tpl):
            with open(tpl, encoding="utf-8") as f:
                data = yaml.safe_load(f)
            if isinstance(data, dict) and data:
                logger.debug("bootstrap: default config from template %s", tpl)
                return data
...
def _write_config(config: dict, num_cells: int) -> dict:
    ...
    try:
        ...
        capture("bootstrap config write failed", error_code="E_BOOTSTRAP_WRITE", component="bootstrap", exc=e)
