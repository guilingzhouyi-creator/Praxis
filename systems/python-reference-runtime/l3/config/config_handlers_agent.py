...
def _safe_max_chars(value: Any) -> int:
    """Coerce a config max_chars value to a non-negative int (0 = default)."""
    try:
...
def cfg_identity_binding(cfg: dict, s: Any, results: dict) -> None:
    """Load praxis.yaml identity_binding: initial role bindings.

    Format: cell_id → {role → {fragment, domain_tags?, max_chars?}}. Loaded
    as a system-internal write (internal=True) so boot-time config works
    without an explicit agent identity; runtime changes go through the
    write gate via /api/v2/identity-binding.
    """
    from l1.kernel.identity_binding import get_identity_binding_manager

    mgr = get_identity_binding_manager()
    count = 0
    for cell_id, roles in (cfg or {}).items():
        ...
        for role, spec in roles.items():
            ...
            r = mgr.bind(
                cell_id,
                role,
                str(spec.get("fragment", "")),
                domain_tags=spec.get("domain_tags") or [],
                max_chars=_safe_max_chars(spec.get("max_chars", 0)),
                internal=True,
            )
            if r.get("success"):
                count += 1
    results["identity_binding"] = count
