...
def cfg_shells(cfg: dict, s: Any, results: dict) -> None:
    """Apply praxis.yaml shells: section (shell family deployment overrides).

    Writes into discovery "shells" (boot_steps/shells.py reads it) and
    SettingsCenter L2 so the family boots with the deployment choices.
    """
    from l1.kernel.discovery import set_config as _set_cfg

    if not isinstance(cfg, dict):
        results["shells"] = False
        return
    if "enabled" in cfg:
        _set_cfg("shells", "enabled", bool(cfg["enabled"]))
        s.set_l2("shells.enabled", bool(cfg["enabled"]))
    if "default" in cfg:
        _set_cfg("shells", "default", str(cfg["default"]))
        s.set_l2("shells.default", str(cfg["default"]))
    if "shells" in cfg and isinstance(cfg["shells"], dict):
        _set_cfg("shells", "shells", cfg["shells"])
    if "bindings" in cfg and isinstance(cfg["bindings"], dict):
        _set_cfg("shells", "bindings", cfg["bindings"])
    results["shells"] = True
