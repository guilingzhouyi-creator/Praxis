...
Each file gets a hash directory under .praxis/resource-buffer/:
        ...
        for hidden_dir in self._root.rglob(RESOURCE_BUFFER_HIDDEN_DIR):
            ...
            for f in hidden_dir.glob(RESOURCE_BUFFER_SLOT_GLOB):
                try:
                    ...
                    # Parallel workers share .praxis/resource-buffer/ — a slot
                    # may have been moved by another process between glob and
                    # move (TOCTOU). Skip silently; the winner restores it.
                    logger.debug("ring: recover move skipped (%s): %s", f.name, exc)
