...
class ApiGateway(ApiHandlers):
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            capture("API gateway failed to start", error_code="E_API_GW_START", component="api_gateway", exc=e)
            logger.error("API gateway failed to start: %s", e)
        except Exception as e:
            from l3.error_bus import capture

            capture("API gateway error", error_code="E_API_GW_ERROR", component="api_gateway", exc=e)
...
def get_gateway() -> ApiGateway | None:
    """Return the API gateway singleton (None before boot)."""
    return _gateway
