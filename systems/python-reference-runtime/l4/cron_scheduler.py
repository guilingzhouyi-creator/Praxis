...
class CronScheduler:
    ...
    def add(
        ...
        ``cell_id`` defaults to the config-driven DEFAULT_CELL_ID when empty
        (never a hardcoded literal in the implementation).
        """
        if not cell_id:
            try:
                from l1.kernel.params.agent import DEFAULT_CELL_ID
    ...
    def _loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                capture("cron tick error", error_code="E_CRON_TICK", component="cron_scheduler", exc=e)
    ...
    def _dispatch(self, entry: dict) -> None:
        ...
        try:
            ...
            capture(
                f"cron dispatch '{entry['id']}' failed",
                error_code="E_CRON_DISPATCH",
