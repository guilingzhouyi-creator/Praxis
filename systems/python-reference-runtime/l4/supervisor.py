        ...
        for role in order:
            ...
            for i in range(cfg.get("replicas", 1)):
                try:
                    ...
                    capture(
                        f"supervisor: failed to start {role}[{i}]",
                        error_code="E_SUPERVISOR_START",
                        component="supervisor",
                        context={"role": role, "replica": i},
                        exc=e,
                    )
                ...
                for i, p in enumerate(procs):
                    if p.poll() is not None:
                        ...
                        try:
                            ...
                            capture(
                                f"supervisor: restart {role}[{i}] failed",
                                error_code="E_SUPERVISOR_RESTART",
                                component="supervisor",
                                context={"role": role, "replica": i},
                                exc=e,
                            )
