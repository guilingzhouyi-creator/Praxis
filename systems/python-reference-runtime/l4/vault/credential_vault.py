...
def init_vault(vault_dir: str = "") -> dict:
    ...
    with _lock:
        ...
        # Derive encryption key from machine-local secret. A key-derivation
        # failure (e.g. unwritable salt file) must fail the init loudly —
        # not silently produce a vault that cannot survive a restart.
        try:
            _VAULT_KEY = _derive_key(data_dir)
        except Exception as e:
            logger.error("credential vault init failed: %s", e)
            return {"success": False, "error": f"vault key derivation failed: {e}"}
    ...
    try:
        if os.path.exists(salt_path):
            ...
            with os.fdopen(fd, "wb") as f:
                f.write(salt)
        # The salt IS the key material: restrict it to the owner even when
        # the file pre-existed with looser permissions.
        os.chmod(salt_path, 0o600)
        return hashlib.sha256(salt).digest()
    except Exception as e:
        # An ephemeral random key would silently orphan every stored
        # credential after restart — fail loudly instead.
        logger.error("credential vault: cannot persist salt file %s: %s", salt_path, e)
        raise
...
def _load_vault() -> None:
    ...
    try:
        ...
        logger.error("credential vault load failed: %s — stored credentials are lost unless the key/file recovers", e)
        _vault = {}
...
def _save_vault() -> bool:
    ...
    try:
        ...
        # mkstemp: 0600 mode + unpredictable name (no symlink-overwrite
        # attack on a fixed .tmp path); os.replace keeps the write atomic.
        import tempfile

        fd, tmp = tempfile.mkstemp(prefix=".vault-", dir=os.path.dirname(_VAULT_PATH) or ".")
