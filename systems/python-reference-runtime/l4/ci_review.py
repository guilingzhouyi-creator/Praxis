...
from .ci_review_models import (  # noqa: F401 — re-export (api_handlers_ci / l2 ci.py)
    CI_SETTING_KEYS,
    CI_SETTING_SUFFIXES,
    CardCiReport,
    _is_allowed_key,
...
class CiReviewService(CiReviewLinkMixin, BaseService):
    ...
    def _do_review(self, card_id: str, state: str, result: dict) -> None:
        ...
        # CI review is an INDEPENDENT entity: it does not drive Agent-bound
        # rework (review_pipeline is invoked by Agent-side review paths, not
        # by CI). Its job is to run gates and feed the result back through
        # the async test-feedback queue (auto_test.push_feedback) so the
        # Cell's peer agents consume it on their next task.
        self._queue_review_feedback(report, result)
        cell_id = str(result.get("cell_id") or "")
        if cell_id:
    ...
    def _run_review_disposition(self, report: CardCiReport, result: dict) -> None:
        """2.1: run the review pipeline over the card's changed files.

        Card-completion linkage: every changed file goes through
        ``review_pipeline.dispose(cell_id=...)`` — small changes get
        directed fixes, large ones route back through the Cell-to-Cell
        channel (auto-topology) with an async L3A report. Structured
        dispositions are merged into the report context for the
        human-facing summary. Degrades gracefully (never blocks review).
        """
        try:
            from l3.services.review_pipeline import get_review_pipeline

            pipe = get_review_pipeline()
            if not pipe.threshold().get("enabled"):
                return
            cell_id = str(result.get("cell_id") or "")
                ...
                if sb is not None:
                    try:
                        ...
                        if entry is not None:
                            hunks = list(getattr(entry, "hunks", []) or [])
                    except Exception:
                        hunks = []
                if not hunks:
                    continue
                from l4.sandbox.sandbox_diff import _hunk_stats

                review_diff = {"hunks": hunks, "stats": _hunk_stats(hunks)}
                d = pipe.dispose(review_diff, rel_path=path, agent_id=agent_id, cell_id=cell_id)
                dispositions.append({"path": path, **d})
            if dispositions:
                report.context["review_pipeline"] = {"dispositions": dispositions}
        except Exception as e:
            logger.debug("ci_review: review disposition skipped: %s", e)

    def _queue_review_feedback(self, report: CardCiReport, result: dict) -> None:
        """CI is an independent entity: queue its review result as async
        test feedback (auto_test.push_feedback) for the Cell's peer agents.

        CI does NOT drive Agent-bound rework — it runs gates and hands the
        outcome back through the feedback queue; the peer agents consume it
        (pop_feedback) on their next task. Non-blocking, degrades to a no-op
        when the queue is unavailable.

        Returns:
            None (feedback is queued asynchronously).
        """
        try:
            from l3.tool_system.auto_test import push_feedback

            agent_id = report.agent_id or str(result.get("agent_id") or result.get("agent") or "")
            if not agent_id:
                return
            push_feedback(
                agent_id,
                {
                    ...
                    "cell_id": str(result.get("cell_id") or ""),
                    "changed_files": list(report.changed_files)[:5],
                    "source": "ci_review",
                },
            )
        except Exception as e:
            logger.debug("ci_review: feedback queue skipped: %s", e)
    ...
    def _run_and_wait(self, card_id: str, steps: list[dict], result: dict) -> tuple[str, list[dict], str]:
        ...
        while time.time() < deadline:
            time.sleep(CI_REVIEW_POLL_INTERVAL)
