...
# ── Three-tier topology views (2.1-D1) ─────────────────────────────────────
# The diff system is a two-faced, three-tier structure:
#   tier 1 (build)    — precise change/new diffs for the building executor
#   tier 2 (review)   — highly structured diff + file context for the review
#                       department (with per-hunk attribution)
#   tier 3 (conflict) — cross-cell conflict diffs
...
def build_diff(
    ...
    """Tier-1 diff for the building executor: precise changes + new files.

    Wraps compute_hunks with a tier marker and a human-readable view so
    the building department sees exactly what it changed (and what is new).

    Returns:
        ``{"tier": "build", "hunks": [...], "unified": str, "stats": {...}}``.
    """
    hunks = compute_hunks(old_text, new_text, agent_id=agent_id, tool_name=tool_name, timestamp=timestamp)
    unified = _hunks_to_unified(hunks, agent_id=agent_id)
    return {
        "tier": "build",
        "agent_id": agent_id,
        "tool_name": tool_name,
        "hunks": hunks,
        "unified": unified,
        "stats": _hunk_stats(hunks),
        # New-file marker: an empty old_text with non-empty new_text means
        # this is a newly created file (build department's "new diff").
        "is_new_file": bool(new_text.strip()) and not bool(old_text.strip()),
    }
...
def review_diff(
    ...
    """Tier-2 diff for the review department: structured hunks + context.

    Enriches the building diff with per-hunk attribution and optional file
    context so the reviewer can reason about the change without reopening
    the file. ``context_lines`` (0 = use DIFF_CONTEXT_LINES) controls how
    much surrounding context is attached.

    Returns:
        ``{"tier": "review", "rel_path": ..., "hunks": [...],
          "attribution": [...], "stats": {...}}``.
    """
    hunks = compute_hunks(old_text, new_text, agent_id=agent_id, tool_name=tool_name, timestamp=timestamp)
    if context_lines and context_lines > 0:
        old_lines = old_text.splitlines(keepends=True)
        for h in hunks:
            ctx_before = max(0, h["original_start"] - 1 - context_lines)
            h["context_before"] = [ln.rstrip("\n") for ln in old_lines[ctx_before : h["original_start"] - 1]]
    attribution = [
        {
            "hunk": i,
            "type": h.get("type", ""),
...
def conflict_diff(
    ...
    """Tier-3 diff: cross-cell conflict detection for the conflict view.

    Wraps check_conflict and returns a structured conflict result that the
    L3A / review department can act on (warn / block / ping_pong).

    Returns:
        ``{"tier": "conflict", "rel_path": ..., "level": ...,
          "actionable": bool}``.
    """
    level = check_conflict(rel_path, agent_id, path_index, entries)
    return {
        "tier": "conflict",
        "rel_path": rel_path,
        "agent_id": agent_id,
        "level": level,
        "actionable": level in ("block", "ping_pong"),
    }
...
def _hunks_to_unified(hunks: list[dict], agent_id: str = "") -> str:
    """Render hunks as a unified diff (human view)."""
    lines: list[str] = [f"--- agent: {agent_id or '?'}", "+++ change"]
    for h in hunks:
        lines.append(
            f"@@ -{h.get('original_start', 0)},{h.get('original_end', 0)} "
            f"+{h.get('modified_start', 0)},{h.get('modified_end', 0)} @@"
        )
...
def _hunk_stats(hunks: list[dict]) -> dict:
    """Aggregate hunk statistics (added/removed/changed counts)."""
    added = sum(len(h.get("added_lines", [])) for h in hunks)
    removed = sum(len(h.get("removed_lines", [])) for h in hunks)
    return {
        "hunks": len(hunks),
        "added_lines": added,
        "removed_lines": removed,
        "changed_lines": added + removed,
        "semantic": {h.get("semantic", ""): 1 for h in hunks},
    }
