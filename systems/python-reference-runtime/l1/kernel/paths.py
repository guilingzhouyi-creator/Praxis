...
def _get_data_dir(mode: DeployMode) -> str:
    ...
    base: dict[DeployMode, str] = {
        ...
        DeployMode.BINARY: str(user_data_dir),
    }
    return base.get(mode, ".praxis")


def _get_install_dir(mode: DeployMode) -> str:
    """Return the program install dir (unpack location) for a deploy mode.

    Per-user unpacked installs (the "installer" layout) keep the read-only
    program in the user space — Windows ``%LOCALAPPDATA%\\Programs``, macOS
    ``/Applications``, POSIX ``~/.local/lib`` — while data stays in the
    platform user-data dir (see ``_get_data_dir``).
    """
    env_val = os.environ.get("PRAXIS_INSTALL_DIR")
    if env_val:
        return env_val
    home = Path.home()
    if mode in (DeployMode.DESKTOP_WIN, DeployMode.BINARY) and IS_WINDOWS:
        return str(Path(os.environ.get("LOCALAPPDATA", str(home))) / "Programs" / "praxis")
    if mode in (DeployMode.DESKTOP_MAC, DeployMode.BINARY) and IS_MAC:
        return "/Applications/Praxis.app"
    if mode == DeployMode.DOCKER:
        return "/app"
    return str(home / ".local" / "lib" / "praxis")


def _get_templates_dir(mode: DeployMode) -> str:
    """Return the shipped config-template source dir for a deploy mode.

    CLI_PROJECT reads the repo's ``config/`` (source checkout); installed
    modes read the ``share/praxis/config`` data-files payload shipped in the
    wheel by the ``praxis-install`` / ``ensure_config`` pipeline.
    """
    env_val = os.environ.get("PRAXIS_TEMPLATES_DIR")
    if env_val:
        return env_val
    if mode == DeployMode.CLI_PROJECT:
        kernel_dir = os.path.dirname(os.path.abspath(__file__))
        return os.path.abspath(os.path.join(kernel_dir, "..", "..", "..", "..", "config"))
    return os.path.join(sys.prefix, "share", "praxis", "config")
...
class PraxisPaths:
    ...
    data_dir: str = ""
    config_dir: str = ""
    # Read-only program unpack location (installer layout; see _get_install_dir).
    install_dir: str = ""
    # Shipped config-template source dir (repo config/ in CLI, wheel data in
    # installed modes) — consumed by ensure_config / praxis-install.
    config_templates_dir: str = ""
    # Log directory (kept separate from config_dir; see B3 layout plan).
    logs_dir: str = ""
    # Enumerable list of runtime directories created by prepare_layout at boot.
    layout_dirs: list[str] = field(default_factory=list)
    ...
    chain_key: str = ""
    archive_db: str = ""
    diff_persist_file: str = ""
    diff_dictionary_file: str = ""
    mcp_state: str = ""
    ...
    def __post_init__(self) -> None:
        ...
        if not self.skill_project_evolved_dir:
            if self.deploy_mode == DeployMode.CLI_PROJECT:
                ...
                # the package root (paths.py is in systems/python-reference-runtime/l1/kernel) so it matches
                # the discovery base used by SkillManager.load_builtin()
                # (kernel_dir/../../../..), not the ephemeral os.getcwd().
                kernel_dir = os.path.dirname(os.path.abspath(__file__))
                self.skill_project_evolved_dir = os.path.abspath(
                    os.path.join(kernel_dir, "..", "..", "..", "..", "skills", "evolved")
                )
            else:
        ...
        if not self.install_dir:
            self.install_dir = _get_install_dir(self.deploy_mode)
        if not self.config_templates_dir:
            self.config_templates_dir = _get_templates_dir(self.deploy_mode)
        if not self.logs_dir:
            self.logs_dir = os.path.join(self.data_dir, "logs")
        if not self.layout_dirs:
            # Single source of truth for the runtime directory layout — boot's
            # prepare_layout step creates exactly these (plus config_dir).
            self.layout_dirs = [
                self.data_dir,
                os.path.join(self.data_dir, "skills"),
                os.path.join(self.data_dir, "skills", "evolved"),
                os.path.join(self.data_dir, "skills", "lean"),
                os.path.join(self.data_dir, "memories"),
                os.path.join(self.data_dir, "sandbox"),
                os.path.join(self.data_dir, "sockets"),
                os.path.join(self.data_dir, "todos"),
                os.path.join(self.data_dir, "l3a-outputs"),
                os.path.join(self.data_dir, "cache"),
                self.logs_dir,
                self.config_dir,
            ]
        # vfs_temp_path historically hardcoded "/tmp" — resolve the real temp
        # dir so Windows/desktop installs get a writable location.
        if self.vfs_temp_path == "/tmp":
            self.vfs_temp_path = tempfile.gettempdir()

        dd = self.data_dir
        self.events_db = os.path.join(dd, "events.db")
        self.state_json = os.path.join(dd, "state.json")
        self.diff_persist_file = os.path.join(dd, "diff_persist.jsonl")
        self.diff_dictionary_file = os.path.join(dd, "diff_dictionary.bin")
