...
class Peer:
    ...
    @property
    def alive(self) -> bool:
        """True if the peer was seen within the timeout window."""
...
class NetKernel:
    ...
    def health(self) -> dict:
        ...
        with self._lock:
            ...
            alive = sum(1 for p in self._peers.values() if p.alive)
    ...
    def _prune_stale_peers_locked(self, now: float) -> None:
        """Report loss once and evict peers gone past NET_PEER_EVICT_AFTER.

        Caller must hold ``self._lock``. Runs on every health() call AND on
        peer announces, so the peer table stays bounded even when health()
        is polled infrequently — otherwise dead peers accumulate until the
        next health() sweep.
        """
        alive = sum(1 for p in self._peers.values() if p.alive)
        for p in self._peers.values():
            if not p.alive and not p._loss_reported:
        ...
        from .params.system import NET_PEER_EVICT_AFTER

        stale = [
            pid for pid, p in self._peers.items() if now - p.last_seen >= NET_PEER_EVICT_AFTER and p._loss_reported
        ]
        for pid in stale:
    ...
    def _on_peer_announce(self, msg: dict) -> None:
        ...
        with self._lock:
            # Opportunistic prune on announce traffic — keeps the table
            # bounded even if health() is polled rarely.
            self._prune_stale_peers_locked(time.time())
