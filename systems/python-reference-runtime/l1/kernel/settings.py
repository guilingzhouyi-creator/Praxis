...
class _FallbackSettings:
    ...
    def set_l2(self, key: str, value: Any) -> dict:
        """Write a configuration-layer setting through the kernel fallback.

        The injected L3 ``SettingsCenter`` exposes ``set_l2`` for config
        handlers.  Standalone kernel/config tests run before that provider is
        wired, so the fallback must expose the same narrow write contract.
        """
