...
# ── Subprocess encoding policy ──

DEFAULT_ENCODING: Final[str] = "utf-8"
"""Explicit text encoding for subprocess pipes — keeps Linux (UTF-8 by
default) behavior identical while avoiding locale-dependent decoding
...
def get_temp_dir() -> str:
    """Return a stable temp directory path for Praxis runtime files.

    Side-effect free: the directory is created lazily by callers that need
    it (via ensure_dir), so importing this module never performs I/O that
    could abort startup on a read-only temp dir.
    """
...
def create_interactive_shell(cwd: str = "") -> _subprocess.Popen:
    ...
    """
    # Bash parses its long options before the short interactive flag; placing
    # ``-i`` first makes GNU bash treat the following long options as an
    # invalid command-line token and exits before the pipe is usable.
    cmd = [SHELL_PATH] if IS_WINDOWS else [SHELL_PATH, "--noprofile", "--norc", "-i"]
    kwargs: dict = {}
    if cwd:
        kwargs["cwd"] = cwd
...
def ensure_dir(path: str) -> str:
    """Create a directory (parents included) and return its absolute path.

    Cross-platform ``mkdir -p``: routes directory creation through one
    helper so runtime data/temp roots are laid out consistently across
    OSes instead of scattered ``os.makedirs`` calls. Idempotent — existing
    directories pass through unchanged.
    """
