...
fn explicit_working_directory_is_deterministic() {
    let check = TerritoryCheck {
        target: "systems/python-reference-runtime/../python-reference-runtime/main.py".to_owned(),
        bases: vec!["systems/python-reference-runtime".to_owned()],
        working_dir: "/workspace/praxis".to_owned(),
    };
    assert!(check.evaluate());
    assert!(!is_within_at(
        "systems/python-reference-runtime/main.py",
        &["/workspace/praxis/systems/python-reference-runtime".to_owned()],
