//! Emit fixed-work bounded SessionBook snapshot-page evidence.

use l1_kernel_rs::benchmark::{BenchmarkEvidence, BenchmarkMetadata, FixedWorkSpec};
use l1_kernel_rs::benchmark_runner::run_session_book_snapshot_page;
...
fn main() {
    let spec = FixedWorkSpec::new("session.book.snapshot_page", 4_096, vec![1, 2, 4], 3)
        .expect("snapshot page benchmark specification is valid");
    let report = run_session_book_snapshot_page(spec, 16, 4_096, 64)
