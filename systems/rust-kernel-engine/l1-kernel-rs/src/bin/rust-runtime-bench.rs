//! Emit a release-mode fixed-work Rust runtime admission report.

use l1_kernel_rs::benchmark::{BenchmarkEvidence, BenchmarkMetadata, FixedWorkSpec};
use l1_kernel_rs::benchmark_runner::run_runtime;
...
fn main() {
    let spec = FixedWorkSpec::new("runtime.submit_reap", 4_096, vec![1, 2, 4], 3)
        .expect("benchmark specification is valid");
    let report = run_runtime(spec).expect("runtime benchmark completed");
