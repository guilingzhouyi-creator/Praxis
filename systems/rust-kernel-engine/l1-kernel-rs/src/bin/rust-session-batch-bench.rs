//! Emit the Rust-native grouped session admission evidence.

use l1_kernel_rs::benchmark::{BenchmarkEvidence, BenchmarkMetadata, FixedWorkSpec};
use l1_kernel_rs::benchmark_runner::run_session_book_batch;
...
fn main() {
    let spec = FixedWorkSpec::new("session.book.batch_admission", 4_096, vec![1, 2, 4], 3)
        .expect("session batch benchmark spec is valid");
    let report = run_session_book_batch(spec, 16, 32).expect("session batch benchmark completed");
