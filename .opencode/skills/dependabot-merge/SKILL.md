...
4. **Merge locally, never on GitHub**: dependabot bot commits are unsigned and GitCode rejects GitHub-side merges. `git merge <branch>` locally (GPG-signed), then push.
5. **Push BOTH remotes**: `bash scripts/sh/push-both.sh main` (origin=GitCode canonical, github=GitHub CI carrier).
...
- Dependabot merges on GitHub UI are outright rejected by GitCode (unsigned commits).
