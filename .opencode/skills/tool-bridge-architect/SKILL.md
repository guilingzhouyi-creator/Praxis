...
- **LLM engine**: providers pluggable; default `ollama`/`qwen2.5-coder:7b` at `localhost:11434`, overridable via `config/praxis.yaml` or env (`OPENAI_API_KEY`, `DEEPSEEK_API_KEY`, `ANTHROPIC_API_KEY`, `OLLAMA_URL`). Provider creds live in the vault, never in code.
