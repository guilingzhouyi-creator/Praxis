...
rust-test-domain:
	...
	cargo test --manifest-path crates/Cargo.toml --test "$(RUST_TEST_DOMAIN)"
...
rust-session-snapshot-page-benchmark:
	@PRAXIS_GIT_REVISION="$${PRAXIS_GIT_REVISION:-$$(git rev-parse --short HEAD 2>/dev/null || printf unknown)}" \
		cargo run --manifest-path crates/Cargo.toml --release --bin rust-session-snapshot-page-bench

rust-session-snapshot-page-contention-benchmark:
	@PRAXIS_GIT_REVISION="$${PRAXIS_GIT_REVISION:-$$(git rev-parse --short HEAD 2>/dev/null || printf unknown)}" \
		cargo run --manifest-path crates/Cargo.toml --release --bin rust-session-snapshot-page-contention-bench

rust-agent-loop-snapshot-page-benchmark:
	@PRAXIS_GIT_REVISION="$${PRAXIS_GIT_REVISION:-$$(git rev-parse --short HEAD 2>/dev/null || printf unknown)}" \
		cargo run --manifest-path crates/Cargo.toml --release --bin rust-agent-loop-snapshot-page-bench

rust-terminal-snapshot-page-benchmark:
	@PRAXIS_GIT_REVISION="$${PRAXIS_GIT_REVISION:-$$(git rev-parse --short HEAD 2>/dev/null || printf unknown)}" \
		cargo run --manifest-path crates/Cargo.toml --release --bin rust-terminal-snapshot-page-bench

rust-registry-base-benchmark:
	@PRAXIS_GIT_REVISION="$${PRAXIS_GIT_REVISION:-$$(git rev-parse --short HEAD 2>/dev/null || printf unknown)}" \
		cargo run --manifest-path crates/Cargo.toml --release --bin rust-registry-base-bench
...
rust-kernel-entry:
	cargo build --manifest-path crates/Cargo.toml --bin rust-kernel-entry
