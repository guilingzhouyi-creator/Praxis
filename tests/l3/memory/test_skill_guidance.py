...
class TestStageStateHygiene:
    """Stage-state dict must stay bounded — TTL prune removes stale entries."""

    def test_prune_removes_stale_entries(self):
        """Entries untouched past SKILL_STAGE_STATE_TTL are dropped."""
        import time as _t

        _mk("ttl-skill", stages=[{"id": "a", "instructions": "A"}, {"id": "b", "instructions": "B"}])
        sm = get_skill_manager()
        sm.current_stage("ttl-skill", "card:old")  # registers a card-scoped entry
        assert ("ttl-skill", "card:old") in sm._stage_state
        # Age the entry past the TTL, then force a prune on the next access
        sm._stage_touched[("ttl-skill", "card:old")] = _t.time() - 999999
        sm._last_stage_prune = 0.0
        sm.current_stage("ttl-skill", "card:other")
        assert ("ttl-skill", "card:old") not in sm._stage_state

    def test_recent_entries_survive_prune(self):
        """Freshly touched entries are retained across prune windows."""
        _mk("keep-skill", stages=[{"id": "a", "instructions": "A"}, {"id": "b", "instructions": "B"}])
        sm = get_skill_manager()
        sm.advance_stage("keep-skill", "agent-1")  # production path: bare agent key
        assert ("keep-skill", "agent-1") in sm._stage_state
        # Force a prune; the entry is recent so it must survive
        sm._last_stage_prune = 0.0
        sm.current_stage("keep-skill", "card:probe")
        assert ("keep-skill", "agent-1") in sm._stage_state

    def test_prune_is_idempotent_without_touched(self):
        """Prune tolerates entries with no touched timestamp (defensive)."""
        _mk("no-touch", stages=[{"id": "a", "instructions": "A"}])
        sm = get_skill_manager()
        sm.current_stage("no-touch", "card:nt")
        sm._stage_touched.pop(("no-touch", "card:nt"), None)  # simulate legacy state
        sm._last_stage_prune = 0.0
        sm.current_stage("no-touch", "card:nt")  # must not raise
        assert sm.current_stage("no-touch", "card:nt")["staged"] is True
