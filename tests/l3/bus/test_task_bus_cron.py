...
class TestTaskBusConcurrency:
    def test_concurrent_dispatch_no_crash(self):
        ...
        class _OkHandler(http.server.BaseHTTPRequestHandler):
            """Accept deliveries so dead-webhook Errno 111 noise is avoided."""
        ...
        # Threaded server + large backlog so concurrent deliveries cannot be
        # refused (a refused delivery would retry 1+4+10 s past any deadline).
        httpd = socketserver.ThreadingTCPServer(("127.0.0.1", 0), _OkHandler)
        httpd.request_queue_size = 128
        threading.Thread(target=httpd.serve_forever, daemon=True).start()
        try:
            reset_task_bus()
            bus = get_task_bus()
            bus.register("c1", f"http://localhost:{httpd.server_address[1]}/h1")
            bus.register("c2", f"http://localhost:{httpd.server_address[1]}/h2")
            bus.register("c3", f"http://localhost:{httpd.server_address[1]}/h3")
