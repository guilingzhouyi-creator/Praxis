...
class TestAgentLoop:
    def test_run(self):
        ...
        # AgentLoop.run() resolves the "llm" port; ensure the mock engine is
        # registered (random-order runs may otherwise execute this test
        # before any other test initializes the LLM engine).
        s = get_settings()
        s.set("llm.provider", "mock")
        try:
            reset_engine()
            get_engine(LLMConfig(provider="mock"))
