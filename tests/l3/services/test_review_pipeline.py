...
def test_small_change_fixed_in_place():
    ...
    r = pipe.dispose(_review_diff(10), rel_path="src/a.py", agent_id="agent-a")
...
def test_large_change_routes_to_rework():
    ...
    pipe = get_review_pipeline()
    r = pipe.dispose(_review_diff(500), rel_path="src/a.py", agent_id="agent-a")
...
def test_large_change_directed_channel_with_cell():
    ...
    pipe = get_review_pipeline()
    try:
        r = pipe.dispose(_review_diff(500), rel_path="src/a.py", agent_id="agent-a", cell_id="cell-2")
        assert r["disposition"] == "large"
        assert r["rework"] is True
        assert r["reported_to_l3a"] is True
        ...
        assert rework[0]["payload"].get("rel_path") == "src/a.py"
    finally:
        reset_bus()
...
def test_dispose_frame_header_large():
    ...
    r = pipe.dispose(_review_diff(5), rel_path="src/a.py", agent_id="agent-a", frame=frame)
    assert r["disposition"] == "large"
    assert r["rework"] is True
...
def test_dispose_frame_header_small():
    ...
    frame = encode_hunks(hunks, frame_type=2)
    pipe = get_review_pipeline()
    r = pipe.dispose(_review_diff(2), rel_path="src/a.py", agent_id="agent-a", frame=frame)
    assert r["disposition"] == "small"
    assert r["rework"] is False
...
def test_dispose_frame_none_unchanged():
    ...
    pipe = get_review_pipeline()
    r = pipe.dispose(_review_diff(2), rel_path="src/a.py", agent_id="agent-a")
    assert r["disposition"] == "small"
