...
class TestRingBufferRecoverRace:
    """Regression: recover() must tolerate a slot moved by a parallel worker (TOCTOU).

    Parallel pytest/xdist workers share the same ``.praxis/resource-buffer/``
    root; two ``recover()`` calls can glob the same ``_hidden/`` slot and race
    to move it. The loser must not crash with FileNotFoundError.
    """

    def _make_hidden_slot(self, root: str) -> tuple[Path, Path]:
        """Create ``<root>/abc/_hidden/slot_0000.dat`` and return (hidden_dir, slot)."""
        hidden = Path(root) / "abc" / "_hidden"
        hidden.mkdir(parents=True, exist_ok=True)
        slot = hidden / "slot_0000.dat"
        slot.write_text("staged content", encoding="utf-8")
        return hidden, slot

    def test_recover_restores_hidden_slot(self):
        """A hidden slot is moved back to _pending and counted."""
        from l3.resource_buffer.ring import RingBuffer

        with tempfile.TemporaryDirectory() as tmpdir:
            buf = RingBuffer(root=tmpdir)
            hidden, slot = self._make_hidden_slot(tmpdir)
            r = buf.recover()
            assert r["success"] is True
            assert r["recovered"] == 1
            assert not slot.exists()
            assert (hidden.parent / "_pending" / "slot_0000.dat").exists()
            buf.stop()

    def test_recover_tolerates_raced_slot(self, monkeypatch):
        """recover() must not raise when another worker moved the slot first."""
        from l3.resource_buffer import ring as ring_mod
        from l3.resource_buffer.ring import RingBuffer

        def _raced_move(src, dst):
            # Simulate the parallel worker that already moved this file.
            raise FileNotFoundError(f"No such file or directory: {src}")

        monkeypatch.setattr(ring_mod.shutil, "move", _raced_move)
        with tempfile.TemporaryDirectory() as tmpdir:
            buf = RingBuffer(root=tmpdir)
            self._make_hidden_slot(tmpdir)
            r = buf.recover()  # must not raise
            assert r["success"] is True
            assert r["recovered"] == 0
            buf.stop()

    def test_recover_tolerates_partial_race(self, monkeypatch):
        """recover() keeps restoring remaining slots after one raced move."""
        from l3.resource_buffer import ring as ring_mod
        from l3.resource_buffer.ring import RingBuffer

        real_move = ring_mod.shutil.move

        def _partial_race(src, dst):
            if src.endswith("slot_0000.dat"):
                raise FileNotFoundError(f"No such file or directory: {src}")
            return real_move(src, dst)

        monkeypatch.setattr(ring_mod.shutil, "move", _partial_race)
        with tempfile.TemporaryDirectory() as tmpdir:
            buf = RingBuffer(root=tmpdir)
            hidden, _ = self._make_hidden_slot(tmpdir)
            (hidden / "slot_0001.dat").write_text("second slot", encoding="utf-8")
            r = buf.recover()  # must not raise
            assert r["success"] is True
            assert r["recovered"] == 1
            assert (hidden.parent / "_pending" / "slot_0001.dat").exists()
            assert not (hidden.parent / "_pending" / "slot_0000.dat").exists()
            buf.stop()
