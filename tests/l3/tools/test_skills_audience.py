...
class TestUseSkill:
    ...
    def test_within_domain_allowed(self):
        ...
        assert r["success"]
        # Structured view: rules/procedures lists, no markdown body (prompt).
        assert "interviewing" in r["description"]
        assert r["rules"] and r["procedures"]
        r2 = use_skill({"name": "tdd"}, "agent-http")
        assert r2["success"]
        assert any("red" in rule for rule in r2["rules"])
