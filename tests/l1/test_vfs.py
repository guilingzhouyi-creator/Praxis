...
class TestVfsVirtual:
    ...
    def test_virtual_unlink(self):
        vfs = VFS()
        vfs.mount("/virtual", MountType.VIRTUAL)
        vfs.write("/virtual/note.txt", "content")
        r = vfs.unlink("/virtual/note.txt")
        assert r["success"]
        assert "/virtual/note.txt" not in vfs._virtual_files

    def test_virtual_unlink_missing(self):
        vfs = VFS()
        vfs.mount("/virtual", MountType.VIRTUAL)
        r = vfs.unlink("/virtual/missing.txt")
        assert not r["success"]
        assert "ENOENT" in r.get("error_code", "")

    def test_virtual_files_capped(self, monkeypatch):
        """Virtual-file dict must not grow past FILE_CACHE_MAX_ENTRIES."""
        monkeypatch.setattr("l1.kernel.params.system.FILE_CACHE_MAX_ENTRIES", 3)
        vfs = VFS()
        vfs.mount("/virtual", MountType.VIRTUAL)
        for i in range(10):
            vfs.write(f"/virtual/f{i}.txt", str(i))
        assert len(vfs._virtual_files) <= 3
...
class TestVfsCacheEviction:
    def test_expired_entry_evicted_on_prune(self):
        """Expired cache entries must be removed, not just skipped."""
        import time as _t

        with tempfile.TemporaryDirectory() as td:
            file_path = os.path.join(td, "x.txt")
            with open(file_path, "w") as f:
                f.write("v1")
            vfs = VFS()
            vfs.mount("/project", MountType.PROJECT, real_path=td)
            assert vfs.read("/project/x.txt")["success"]
            assert "/project/x.txt" in vfs._cache
            # Age the entry past its TTL, then read a second file to trigger prune
            cached = vfs._cache["/project/x.txt"]
            vfs._cache["/project/x.txt"] = (cached[0], _t.time() - 1)
            with open(file_path, "w") as f:
                f.write("v2")
            r = vfs.read("/project/x.txt")
            assert r["success"]
            assert r["content"] == "v2"
            assert "/project/x.txt" in vfs._cache  # re-cached fresh
            assert vfs._cache["/project/x.txt"][1] > _t.time()

    def test_cache_capped(self, monkeypatch):
        """Cache dict must not grow past FILE_CACHE_MAX_ENTRIES."""
        monkeypatch.setattr("l1.kernel.params.system.FILE_CACHE_MAX_ENTRIES", 3)
        with tempfile.TemporaryDirectory() as td:
            vfs = VFS()
            vfs.mount("/project", MountType.PROJECT, real_path=td)
            for i in range(10):
                with open(os.path.join(td, f"f{i}.txt"), "w") as f:
                    f.write(str(i))
                vfs.read(f"/project/f{i}.txt")
            assert len(vfs._cache) <= 3
