...
class TestLedgerRecord:
    ...
    def test_index_keys_bounded(self, monkeypatch):
        """Index dict keys must be pruned past LEDGER_MAX_INDEX_KEYS (memory hygiene)."""
        import l1.kernel.gatechain as gc_mod

        monkeypatch.setattr(gc_mod, "LEDGER_MAX_INDEX_KEYS", 3)
        ledger = ToolHistoryLedger(max_entries=10)
        for i in range(10):
            ledger.record(LedgerEntry(agent_id=f"agent-{i}", tool=f"tool-{i}", target=f"/{i}", result=GateResult.PASS))
        assert len(ledger._by_agent_tool) <= 3
        assert len(ledger._by_agent) <= 3
        assert len(ledger._by_tool) <= 3
        # The global entries deque is unaffected
