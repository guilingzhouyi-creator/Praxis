...
class TestEnsureDir:
    """Verify ensure_dir creates nested dirs idempotently and normalizes paths."""
...
class TestRunShell:
    ...
    def test_utf8_roundtrip(self):
        """Non-ASCII output survives the pipe (explicit utf-8 policy)."""
        # PYTHONUTF8 forces the child's stdout to utf-8 regardless of locale
        # / codepage (Windows ANSI code page would raise UnicodeEncodeError
        # for CJK), making the round-trip deterministic across platforms.
        r = run_shell(f"PYTHONUTF8=1 {PYTHON_EXE} -c \"print('praxis-编码-测试')\"")
        assert r.returncode == 0
        assert "编码" in r.stdout

    def test_run_shell_defaults_utf8_encoding(self, monkeypatch):
        ...
        run_shell("echo x")
        assert captured["encoding"] == DEFAULT_ENCODING
        assert captured["errors"] == "replace"
        assert captured["text"] is True

    def test_run_args_defaults_utf8_encoding(self, monkeypatch):
        import subprocess as _subprocess
