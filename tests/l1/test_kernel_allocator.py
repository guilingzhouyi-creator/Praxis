...
class TestConcurrentOOM:
    """Concurrency regression: cross-agent OOM under parallel shard traffic."""

    def test_oom_across_shards_no_dict_race(self):
        """Parallel agents exhausting resources must not crash or deadlock.

        Regression: _oom_kill used to iterate the shared allocation dicts
        under only the global lock while other agents' alloc/free mutated the
        same dicts under their shard locks — a race that could raise
        "RuntimeError: dictionary changed size during iteration" exactly when
        the allocator is under pressure.
        """
        import threading
        ...
        reset_allocator()
        a = get_allocator()
        agent_ids = [f"oom_agent_{i}" for i in range(4)]
        for aid in agent_ids:
            a.set_limit(aid, "tokens", 2)
            a.set_limit(aid, "priority", 1)

        errors: list[Exception] = []

        def alloc_loop(aid: str) -> None:
            # Never free: used saturates at the tiny limit, so every further
            # alloc hits the OOM path and reclaims from another agent.
            try:
                for _ in range(60):
                    a.alloc(aid, "tokens", 1)
            except Exception as e:  # pragma: no cover - failure path
                errors.append(e)

        def scan_loop() -> None:
            # Cross-agent iterators that read the shared dicts under the
            # global lock while the allocators mutate them under shards.
            try:
                for _ in range(50):
                    a.pressure()
                    a.summary()
        ...
        threads = [threading.Thread(target=alloc_loop, args=(aid,)) for aid in agent_ids]
        threads += [threading.Thread(target=scan_loop) for _ in range(2)]
        for t in threads:
            t.start()
        for t in threads:
            t.join(timeout=15)
        alive = [t.name for t in threads if t.is_alive()]
        assert not alive, f"deadlock: threads still running: {alive}"
        assert not errors, f"exceptions in threads: {errors}"
        # Sanity accounting: an agent's usage may exceed its own limit by the
        # amount reclaimed from an OOM victim (borrowed tokens), so the bound
        # is deliberately loose — it only catches gross double-counting, not
        # legitimate cross-agent reclaim.
        for aid in agent_ids:
            u = a.usage(aid)
            assert 0 <= u["tokens"]["used"] <= u["tokens"]["limit"] + 8
