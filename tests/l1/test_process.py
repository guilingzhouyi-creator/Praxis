...
def pt():
    ...
    for pcb in list(table._processes.values()):
        table.reap(pcb.pid)
    table.stop()  # halt the zombie-reaper thread (no per-test thread leak)
...
class TestProcessTableInit:
    ...
    def test_reset_stops_previous_reaper_thread(self):
        """reset_table() must halt the old table's zombie-reaper thread (no leak)."""
        t = get_table()
        old_thread = t._gc_thread
