...
class TestEdgeCases:
    ...
    def test_set_profile_updates_all_usage(self):
        ...
        lim = ResourceLimiter()
        # Use a custom agent: set_profile mutates the ResourceProfile object
        # in place, and the limiter's _profiles is a shallow copy of
        # DEFAULT_PROFILES — targeting an existing profile like "default"
        # would leak max_workers=10 into every later test (randomized-order
        # breakage, see test_usage_after_checks / test_check_workers_success).
        lim.set_profile("custom-default", max_workers=10)
        all_u = lim.all_usage()
        assert all_u["custom-default"]["workers"]["max"] == 10
