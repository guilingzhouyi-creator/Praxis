...
class TestCatalogAudienceAndDisclosure:
    ...
    def test_l3a_session_sees_strategy_and_capability_view(self):
        ...
        # Which strategy skill wins the limited curated slots depends on
        # load order / relevance ranking (environment-sensitive), so assert
        # domain visibility — at least one strategy-tagged skill must surface
        # for the decision layer — rather than a specific name.
        skill_lines = [line for line in _catalog_lines(out) if not line.strip().startswith("[peer-capability]")]
        names = {line.split(":", 1)[0].strip().split(" ")[0] for line in skill_lines}
        assert names & {"ask-matt", "grill-me", "grilling", "handoff", "writing-for-agents"}
        # Execution-domain skills are filtered out of the curated skills
        # section; they surface only as [peer-capability] entries (the
        # capability view is not the strategy catalog).
        assert "tdd" not in names
        assert "tdd" in out  # execution skill visible via capability view only
