...
class TestNetKernel:
    ...
    def test_health_evicts_stale_peers(self):
        """Dead peers past NET_PEER_EVICT_AFTER are removed from the table."""
        import time
        ...
        nk = NetKernel(transport=MockTransport())
        now = time.time()
        nk._peers["fresh"] = Peer(id="fresh", host="10.0.0.2", port=1111, last_seen=now)
        # Stale peer: loss already reported, last announce far in the past
        nk._peers["stale"] = Peer(id="stale", host="10.0.0.3", port=2222, last_seen=now - 9999, _loss_reported=True)
        h = nk.health()
        assert "fresh" in nk._peers
        assert "stale" not in nk._peers
        assert h["peers_total"] == 1

    def test_health_keeps_recent_dead_peer(self):
        """A peer within the eviction grace period is retained (loss reported once)."""
        import time
        ...
        nk = NetKernel(transport=MockTransport())
        now = time.time()
        # Dead (beyond timeout) but inside the eviction grace period
        nk._peers["recent"] = Peer(id="recent", host="10.0.0.4", port=3333, last_seen=now - 100)
        nk.health()
        assert "recent" in nk._peers

    def test_announce_evicts_stale_peers(self):
        """Announce traffic prunes stale peers even without a health() call."""
        import time
        ...
        nk._node_id = "self-node"
        now = time.time()
        nk._peers["stale"] = Peer(id="stale", host="10.0.0.9", port=9999, last_seen=now - 9999, _loss_reported=True)
        # A fresh announce triggers the opportunistic prune — no health() needed
        nk._on_peer_announce({"peer_id": "new-peer", "host": "10.0.0.5", "port": 7777, "cells": 1})
        assert "stale" not in nk._peers
        assert "new-peer" in nk._peers
