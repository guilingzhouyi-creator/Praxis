...
def _clean_tool_registry():
    ...
    These tests register their own tools and assert on registry state; other
    files' tests register tools too, so under parallel (xdist) scheduling
    the shared registry can be polluted across files. Saving/clearing/
    restoring around each test makes the file self-contained (same pattern
    as tests/l3/tools/test_config.py).
    """
...
class TestToolAgentPipelineIntegration:
    ...
    # Known pre-existing flake under parallel scheduling (inter-file
    # TOOL_REGISTRY pollution; root cause not fully isolated despite the
    # _clean_tool_registry fixture). Rerun before failing — a true
    # regression still fails after the reruns.
