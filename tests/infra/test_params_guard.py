...
def test_orphan_constants_do_not_grow() -> None:
    ...
    assert not unknowns, f"constants dead but not in ledger (add to BASELINE_DEAD_ORPHANS): {sorted(unknowns)}"
    ...
    excess = known - ledger
    assert not excess, f"new dead constants beyond known ledger: {sorted(excess)}"
