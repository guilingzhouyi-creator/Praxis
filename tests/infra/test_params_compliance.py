...
def _bare_slices(
    ...
    constant appears elsewhere in the file. The exemption matches the
    constant as a whole token (word boundary), so a longer constant name
    (e.g. ``LOG_TRUNC_2000``, ``HASH_TRUNC_SHORTEST``) cannot mask a bare
    slice that should use a shorter-prefixed constant (``LOG_TRUNC_200`` /
    ``HASH_TRUNC_SHORT``) on the same line.
    """
    ...
    for line_no, line in enumerate(content.splitlines(), 1):
        for m in pattern.finditer(line):
            ...
            if val not in values:
                continue
            const = values[val]
            if re.search(rf"\b{re.escape(const)}\b", line):
                continue
...
class TestParamConstantsExist:
    ...
    def test_resource_buffer_dir_constants_exist(self):
        ...
        assert RESOURCE_BUFFER_ROOT_DIR == "resource-buffer"
