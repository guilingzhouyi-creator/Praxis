...
HUNK_AUDIT = ROOT / "scripts" / "py" / "check_sensitive_paths.py"
...
def _install_gates(repo: Path) -> None:
    ...
    shutil.copy2(HUNK_AUDIT, repo / "scripts" / "py" / "check_sensitive_paths.py")
...
def test_requires_hunk_audit_script(scratch: Path) -> None:
    ...
    _git(scratch, "switch", "-q", "feature/tiny")
    (scratch / "scripts" / "py" / "check_sensitive_paths.py").unlink()
    res = _run(scratch)
    assert res.returncode == 3
