...
class TestMCPStateMachine:
    ...
    def test_bridge_init_empty(self):
        from l4 import mcp_state as ms

        orig_path = ms.MCP_STATE_PATH
        with tempfile.TemporaryDirectory() as d:
            ms.MCP_STATE_PATH = os.path.join(d, "mcp_state.json")
            try:
...
class TestMCPStatePersistence:
    def test_save_and_load_state(self, tmp_path):
        ...
        import l4.mcp_state as ms
        from l4.mcp_state import (
            _load_mcp_state,
            _save_mcp_state,
        )

        original_path = ms.MCP_STATE_PATH
        try:
            test_path = os.path.join(str(tmp_path), "mcp_state.json")
            ms.MCP_STATE_PATH = test_path
            ...
            ms.MCP_STATE_PATH = original_path
