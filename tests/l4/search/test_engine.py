...
class TestSearchHandlers:
    """API handlers — root_dir 越界防护（防任意文件读取）。"""

    def test_handle_search_rejects_escape(self):
        """root 指向项目外（如 /etc）必须被拒绝。"""
        from l4.search.search_engine import handle_search

        r = handle_search({"query": "passwd", "root": "/etc"})
        assert not r.get("success")
        assert "outside allowed" in r.get("error", "")

    def test_handle_search_semantic_rejects_escape(self):
        """semantic 入口同样拒绝越界 root。"""
        from l4.search.search_engine import handle_search_semantic

        r = handle_search_semantic({"query": "passwd", "root": "/etc"})
        assert not r.get("success")
        assert "outside allowed" in r.get("error", "")

    def test_handle_search_accepts_cwd_subdir(self):
        """项目内（cwd 之下）的 root 应被接受。"""
        import os
        import shutil
        import tempfile
        ...
        probe = tempfile.mkdtemp(dir=os.getcwd(), prefix="search_probe_")
        try:
            r = handle_search({"query": "hello_world", "root": probe, "pattern": "*.py"})
