...
class TestReviewDisposition:
    """2.1 — card-completion linkage: review pipeline over changed files."""

    def _fake_sandbox(self):
        class _FakeEntry:
            hunks = [
                {
                    "type": "replace",
    ...
    def _fake_pipeline(self):
        class _FakePipe:
            ...
            def dispose(self, review_diff, rel_path="", agent_id="", cell_id=""):
                return {
                    "disposition": "large",
                    "changed_lines": 4,
                    "fixed": False,
                    "rework": True,
                    "reported_to_l3a": True,
                    "path": rel_path,
                }

        return _FakePipe()

    def test_disposition_merges_into_report(self, tmp_path, monkeypatch):
        import l3.services.review_pipeline as rp_mod
        from l4 import sandbox as sb_mod
        from l4.ci_review import CiReviewService

        monkeypatch.setattr(sb_mod, "get_manager", self._fake_sandbox)
    ...
    def test_disposition_skipped_when_pipeline_disabled(self, tmp_path, monkeypatch):
        import l3.services.review_pipeline as rp_mod
        from l4 import sandbox as sb_mod
        from l4.ci_review import CiReviewService
    ...
    def test_disposition_skipped_without_hunks(self, tmp_path, monkeypatch):
        import l3.services.review_pipeline as rp_mod
        from l4 import sandbox as sb_mod
        from l4.ci_review import CiReviewService
        ...
        class _NoHunkSandbox:
            def get_entry(self, path):
                return _NoHunkEntry()

        class _NoHunkManager:
            def get_cell(self, cell_id):
                return _NoHunkSandbox()

        monkeypatch.setattr(sb_mod, "get_manager", lambda: _NoHunkManager())
        monkeypatch.setattr(rp_mod, "get_review_pipeline", self._fake_pipeline)
        svc = CiReviewService(persist_path=str(tmp_path / "x.jsonl"))
    ...
    def test_disposition_degrades_on_error(self, tmp_path, monkeypatch):
        import l3.services.review_pipeline as rp_mod
        from l4 import sandbox as sb_mod
        from l4.ci_review import CiReviewService
...
class TestCIReviewFeedback:
    """CI is an independent entity: review results go back via the async
    test-feedback queue (auto_test.push_feedback), not Agent-bound rework."""

    def test_queue_review_feedback_pushes(self, tmp_path):
        ...
        assert fb and fb[0]["card_id"] == "card-fb"
        assert fb[0]["verdict"] == "PASS"
        assert fb[0]["source"] == "ci_review"

    def test_queue_review_feedback_no_agent_noop(self, tmp_path):
        from l3.tool_system.auto_test import pop_feedback
        from l4.ci_review import CiReviewService

        svc = CiReviewService(persist_path=str(tmp_path / "x.jsonl"))
        report = _make_report("card-fb2", "PASS", agent_id="")
        svc._queue_review_feedback(report, {"cell_id": "cell-1"})  # no agent → no-op
        # Nothing was queued for this agent (pop by exact agent, not global).
        assert pop_feedback("agent-writer") == []

    def test_queue_review_feedback_degrades(self, tmp_path, monkeypatch):
        from l4.ci_review import CiReviewService

        def _boom(*args, **kwargs):
            raise RuntimeError("queue down")

        # _queue_review_feedback imports push_feedback from auto_test at call
        # time — patch the module attribute, not ci_review.
        monkeypatch.setattr("l3.tool_system.auto_test.push_feedback", _boom)
        svc = CiReviewService(persist_path=str(tmp_path / "x.jsonl"))
        report = _make_report("card-fb3", "PASS", agent_id="agent-writer")
        svc._queue_review_feedback(report, {"cell_id": "cell-1"})  # must not raise
