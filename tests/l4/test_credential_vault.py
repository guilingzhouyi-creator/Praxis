...
class TestVaultKeyDerivation:
    """vault 密钥派生 — salt 权限与失败路径。"""

    def test_salt_file_created_0600(self, tmp_path):
        """新 salt 文件必须以 0600 创建（salt 即密钥材料，不能世界可读）。"""
        import hashlib

        import l4.vault.credential_vault as cv
        from l1.kernel.params.system import VAULT_SALT_FILENAME

        seed_dir = tmp_path / "seed"
        seed_dir.mkdir()
        salt_path = tmp_path / VAULT_SALT_FILENAME
        key = cv._derive_key(str(seed_dir))
        assert salt_path.exists()
        assert (salt_path.stat().st_mode & 0o777) == 0o600
        assert key == hashlib.sha256(salt_path.read_bytes()).digest()

    def test_derive_key_fails_loudly_on_write_error(self, tmp_path, monkeypatch):
        """salt 写入失败必须抛错，而不是静默生成随机密钥（重启后凭据全丢）。"""
        import pytest

        import l4.vault.credential_vault as cv

        def _deny(*args, **kwargs):
            raise PermissionError("salt dir not writable")

        seed_dir = tmp_path / "seed"
        seed_dir.mkdir()
        monkeypatch.setattr(cv.os, "open", _deny)
        with pytest.raises(PermissionError):
            cv._derive_key(str(seed_dir))
