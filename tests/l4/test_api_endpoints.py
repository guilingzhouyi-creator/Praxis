...
class TestGenericRegistration:
    ...
    def test_register_endpoint_infers_group(self):
        from l4.api.api_endpoints import register_domain, register_endpoint

        # Self-contained: register_domain is idempotent (setdefault), so
        # declaring the mapping here must not depend on test_register_domain
        # having run first — randomized order breaks that assumption (an
        # unregistered domain would fall back to the "misc" group).
        register_domain("workflow", "tools")
